...
Java_com_example_myapplication_MainActivity_Run_1LLM(JNIEnv *env, jclass clazz, jstring jquery,
    ...
    if (add_prompt) {
        ...
        if (use_think) {
            ...
            get_ids.insert(get_ids.begin(), {151644, 872, 198});
            get_ids.insert(get_ids.end(), {151645, 198, 151644, 77091, 198, 151667, 271, 151668, 271});
        }
    ...
    if (chatting) {  // Java multithreading may not stop immediately. Therefore, use a switch to prevent over runs.
        ...
        if ((token_id != end_id_0) && (token_id != end_id_1) && (response_count < single_chat_limit) && (history_len < max_seq_len)) {
            ...
            response_count += 1;
            if (token_id == 151667) {
                return env->NewStringUTF("正在思考 Thinking...\n--------------------");
            } else if (token_id == 151668) {
                return env->NewStringUTF("--------------------\n思考结束 End of thinking.\n");
            }
            ...
            response_count += 1;
            save_max_logit_position[response_count] = 198;
            response_count += 1;
