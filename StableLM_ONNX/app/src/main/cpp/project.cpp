...
Java_com_example_myapplication_MainActivity_Pre_1Process(JNIEnv *env, jobject clazz) {
    ...
    for (int i = 1; i < theta.size(); i++) {
        theta[i] = std::powf(10000.f, -theta[i] * 0.0625f);  // 1/(10000^(x/16))
    }
    theta[0] = 1.f;
...
Java_com_example_myapplication_MainActivity_Run_1LLM(JNIEnv *env, jclass clazz,
                                                     jint count_words,
                                                     jintArray jquery_ids,
                                                     jboolean add_prompt,
                                                     jboolean clear) {
    if (add_prompt) {
        clear_history();  // Clear every time, because the second round chat not perform well currently.
        jint* jinput_ids = env->GetIntArrayElements(jquery_ids, nullptr);
        std::vector<int32_t> get_ids(jinput_ids, jinput_ids + count_words);
        env->ReleaseIntArrayElements(jquery_ids, jinput_ids, 0);
...
Java_com_example_myapplication_MainActivity_Load_1Models_1A(JNIEnv *env, jobject clazz,
    ...
    OrtSessionOptions *session_options_A;
    {
        ...
        ort_runtime_A->AddSessionConfigEntry(session_options_A, // Binding the #cpu to run the model. 'A;B;' means A & B work respectively. 'A,B' means A & B work cooperatively.
                                             ...
                                             "1,3;2,4");  // It is the best cost/performance (C/P) value setting for running the StableLM2-1.6B-Chat LLM on the Kirin 990 5G, due to limitations imposed by the RAM bandwidth.
