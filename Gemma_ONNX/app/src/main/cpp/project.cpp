...
Java_com_example_myapplication_MainActivity_Load_1Models_1A(JNIEnv *env, jobject clazz,
    ...
    OrtSessionOptions *session_options_A;
    {
        ...
        ort_runtime_A->AddSessionConfigEntry(session_options_A, // Binding the #cpu to run the model. 'A;B;' means A & B work respectively. 'A,B' means A & B work cooperatively.
                                             ...
                                             "1;2");  // It is the best cost/performance (C/P) value setting for running the Gemma1.1-2B LLM on the Kirin 990 5G, due to limitations imposed by the RAM bandwidth.
...
Java_com_example_myapplication_MainActivity_Load_1Models_1C(JNIEnv *env, jobject thiz,
    ...
    OrtSessionOptions *session_options_C;
    {
        ...
        ort_runtime_C->CreateRunOptions(&run_options_A);
        ort_runtime_C->AddRunConfigEntry(run_options_A, "memory.enable_memory_arena_shrinkage", "");  // Keep empty for performance; "cpu:0" for low memory usage.
        ort_runtime_C->AddRunConfigEntry(run_options_A, "disable_synchronize_execution_providers", "1");  // 1 for aggressive performance.
    ...
    ort_runtime_C->CreateTensorWithDataAsOrtValue(
            memory_info,
            reinterpret_cast<void*>(reinterpret_cast<float*> (hidden_states_C.data())), static_cast<size_t> (hidden_size) * sizeof(float),
