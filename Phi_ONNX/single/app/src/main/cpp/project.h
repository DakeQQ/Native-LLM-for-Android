...
int64_t ids_len = 0;
//Ort::Float16_t attention_mask = Ort::Float16_t(-65504.f);             // Enable if using all fp16 format, and project.cpp line: 71, 158, 169, 409
float attention_mask = -65504.f;                                        // Disable if using all fp16 format, and project.cpp line: 72, 159, 170, 410
const std::string file_name_A = "Phi3_mini_1024_A.onnx";
const std::string file_name_A_external = "NONE";                        // If using external data to load the model, provide the file name; otherwise, set to "NONE". If contains many parts, please modify the project.cpp line 280-282.
...
const int single_chat_limit = max_token_history / 2 - 1;                // It is recommended to set it to max_token_history/3, and use phrases like 'go ahead', 'go on', or 'and then?' to continue answering."
...
const std::string vocab_file = "/data/user/0/com.example.myapplication/cache/vocab_Phi.txt";  // We have moved the vocab.txt from assets to the cache folder in Java process.
