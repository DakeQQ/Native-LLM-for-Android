...
std::string Tokenizer::decode(const std::vector<int>& ids) {
    std::string result;
    result.reserve(ids.size() * 4);   // most sub-words decode to a few bytes; avoid repeated regrowth
...
std::vector<int> Tokenizer::encode(const std::string& str) {
    ...
    if (!special_tokens_cache_.empty()) {
        ...
        if (start < n) {
            if (start == 0) {
                encode(str, ids);   // no special matched anywhere: encode the original, no copy
            } else {
...
void Tiktoken::encode(const std::string& str, std::vector<int>& ids) {
    ...
    ids.reserve(ids.size() + str.size() / 3 + 1);
...
class ReplaceDecoder : public TokenDecoder {
    std::string pattern_, content_;
public:
    ReplaceDecoder(std::string  p, const std::string& c) : pattern_(std::move(p)), content_(c) {}
...
class StripDecoder : public TokenDecoder {
    ...
    int start_, stop_;
public:
    StripDecoder(std::string  c, int start, int stop) : content_(std::move(c)), start_(start), stop_(stop) {}
    void decode_chain(std::vector<std::string>& tokens) const override {
...
public:
    void decode_chain(std::vector<std::string>& tokens) const override {
        ...
        for (auto& t : tokens) {
            std::string out;
            for (uint32_t i = 0; i < t.length(); ) {
                const auto* tp = (const uint8_t*)t.c_str();
...
class WordPieceDecoder : public TokenDecoder {
    ...
    bool cleanup_;
public:
    WordPieceDecoder(std::string  prefix = "##", bool cleanup = true) : prefix_(std::move(prefix)), cleanup_(cleanup) {}
...
class MetaspaceDecoder : public TokenDecoder {
    ...
    bool add_prefix_space_;
public:
    MetaspaceDecoder(std::string  rep = "\xe2\x96\x81", bool aps = true) : replacement_(std::move(rep)), add_prefix_space_(aps) {}
    void decode_chain(std::vector<std::string>& tokens) const override {
        for (auto& t : tokens) {
            std::string out;
            uint32_t i = 0;
...
static inline StringView fast_getline(const char*& ptr, const char* end) {
    ...
    while (ptr < end && *ptr != '\n') ++ptr;
    StringView sv = {start, (uint32_t)(ptr - start)};
    if (ptr < end) ++ptr; // skip '\n'
    return sv;
}
