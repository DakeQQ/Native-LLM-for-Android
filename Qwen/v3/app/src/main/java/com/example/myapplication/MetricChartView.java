...
public class MetricChartView extends View {
    ...
    public MetricChartView(Context context, AttributeSet attrs) {
        ...
        gridColor = ContextCompat.getColor(context, R.color.chartGrid);
        axisTextColor = ContextCompat.getColor(context, R.color.chartAxisText);
        panelFillColor = ContextCompat.getColor(context, R.color.chartPanelFill);
        panelStrokeColor = ContextCompat.getColor(context, R.color.chartPanelStroke);
        emptyTextColor = ContextCompat.getColor(context, R.color.chartAxisText);
        ...
        unitTextPaint.setTextSize(sp(13.5f));        unitTextPaint.setTypeface(Typeface.DEFAULT_BOLD);

        panelPaint.setStyle(Paint.Style.FILL);
        panelPaint.setColor(panelFillColor);
        panelStrokePaint.setStyle(Paint.Style.STROKE);
        panelStrokePaint.setStrokeWidth(dp(1f));
        panelStrokePaint.setColor(panelStrokeColor);
    ...
    public void configureSeries(int index, String unit,
        ...
        s.unit = unit == null ? "" : unit;
        s.unitLabel = s.unit.trim();
        s.color = color;
    ...
    public void setData(MetricsHistory.Series a, MetricsHistory.Series b) {
        setData(a, b, true);
    }
    ...
    public int seriesColor(int index) {
    ...
    private void drawGridAndAxes(Canvas canvas, float visMinX, float visMaxX) {
        ...
        if (hasLeftRange) {
            visibleYRange(left, visMinX, visMaxX, leftAxisRange);
        }
        if (hasRightRange) {
            visibleYRange(right, visMinX, visMaxX, rightAxisRange);
        }
        ...
        for (int i = 0; i <= GRID_LINES; i++) {
            ...
            if (hasLeftRange) {
                float val = leftAxisRange[0] + fb * (leftAxisRange[1] - leftAxisRange[0]);
                axisTextPaint.setColor(leftLabelColor);
                axisTextPaint.setTextAlign(Paint.Align.RIGHT);
                canvas.drawText(compact(val), plot.left - axisLabelPadX, y + axisLabelBaseline, axisTextPaint);
            }
            if (hasRightRange) {
                float val = rightAxisRange[0] + fb * (rightAxisRange[1] - rightAxisRange[0]);
                axisTextPaint.setColor(rightLabelColor);
                axisTextPaint.setTextAlign(Paint.Align.LEFT);
                canvas.drawText(compact(val), plot.right + axisLabelPadX, y + axisLabelBaseline, axisTextPaint);
            }
        }
        ...
        for (int i = 0; i <= 4; i++) {
            ...
            float val = visMinX + sf * (visMaxX - visMinX);
            String text = xIsTime ? formatTime(val) : compact(val);
            canvas.drawText(text, x, xLabelBaseline, axisTextPaint);
        }
    }
    ...
    private void drawSeries(Canvas canvas, SeriesState s, float dMinX, float spanX,
        ...
        visibleYRange(s, visMinX, visMaxX, drawYRange);
        ...
        for (int i = 0; i < n; i++) {
            float nx = (xs[i] - dMinX) / spanX;
            float sf = (nx - viewLo) / viewSpan;
            pxBuf[i] = plot.left + sf * plot.width();
            float fy = (ys[i] - yLo) / ySpan;
            pyBuf[i] = plot.bottom - fy * plot.height();
        }
