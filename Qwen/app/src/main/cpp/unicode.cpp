...
namespace MNN::Unicode {

#define TOKENIZER_LOGW(...) __android_log_print(ANDROID_LOG_WARN, "Tokenizer", __VA_ARGS__)
...
Category get_category(int32_t cp) {
    ...
    auto ucp = (uint32_t)cp;
    uint32_t lo = 0, hi = (uint32_t)kCategoryRangesSize;
    while (lo < hi) {
...
int32_t to_lower(int32_t cp) {
    uint32_t lo = 0, hi = (uint32_t)kToLowerSize;
    while (lo < hi) {
        uint32_t mid = (lo + hi) / 2;
...
static std::string fold_text(const std::string& text, std::vector<uint32_t>& byte_offsets) {
    std::string folded;
    byte_offsets.clear();
...
public:
    ...
    std::vector<std::vector<RNode>> parse() {
        ...
        if (supported_) {
            ...
            TOKENIZER_LOGW("skipping unsupported regex branch in pattern");
        }
...
static const char* kGPT2Pattern = R"('s|'t|'re|'ve|'m|'ll|'d| ?\p{L}+| ?\p{N}+| ?[^\s\p{L}\p{N}]+|\s+(?!\S)|\s+)";
...
static const CompiledPattern& get_compiled(const std::string& pattern) {
    ...
    if (it == cache.end()) {
        ...
        if (cp.branches.empty()) {
            TOKENIZER_LOGW("all branches unsupported in pattern; falling back to GPT-2 pattern");
...
static std::wstring utf8_to_wstring(const std::string& s, std::vector<uint32_t>& byte_offsets) {
    std::wstring ws;
    byte_offsets.clear();
    const auto* p = (const uint8_t*)s.data();
    uint32_t len = (uint32_t)s.size(), i = 0;
    while (i < len) {
        byte_offsets.push_back(i);
        int32_t cp;
...
static std::vector<std::pair<uint32_t, uint32_t>> unicode_regex_find_all(const std::string& text, const std::string& pattern) {
    ...
    for (auto it = begin; it != end_it; ++it) {
        uint32_t ws = (uint32_t)it->position();
        uint32_t we = ws + (uint32_t)it->length();
        matches.emplace_back(byte_offsets[ws], byte_offsets[we]);
    }
    return matches;
}
