...
public class ComputeMemoryActivity extends AppCompatActivity {
    ...
    private static final float[] EMPTY_FLOATS = new float[0];
        ...
        private static final BenchmarkController BENCHMARK_CONTROLLER = new BenchmarkController();
        private static WeakReference<ComputeMemoryActivity> activeActivity =
    ...
    private static final float DYNAMIC_QUANT_GROUP_SIZE = 32.0f;
    private static final float Q4F32_BYTES_PER_WEIGHT =
            0.5f + (4.0f + 0.5f) / DYNAMIC_QUANT_GROUP_SIZE;
    ...
    // Run the benchmark through a process-level controller so Activity recreation cannot start a second
    // native task while the first one still owns hundreds of MiB of benchmark arrays.
    private void startBenchmark() {
        if (BENCHMARK_CONTROLLER.isRunning()) {
            renderBenchmarkState();
            return;
        }
        ...
        BENCHMARK_CONTROLLER.start(arrayMillions, iterations, littleThreads, bigThreads);
        renderBenchmarkState();
    }

    private void renderBenchmarkState() {
        boolean running = BENCHMARK_CONTROLLER.isRunning();
        runButton.setEnabled(!running);
        runProgress.setVisibility(running ? View.VISIBLE : View.GONE);
        if (running) {
            resultContainer.setVisibility(View.GONE);
            resultStatus.setVisibility(View.VISIBLE);
        ...
        String report = BENCHMARK_CONTROLLER.getReport();
        if (report != null) {
            showResult(report);
    ...
    private static final class BenchmarkController {
        ...
        synchronized void start(int arrayMillions, int iterations,
                                int littleThreads, int bigThreads) {
            if (running) {
                return;
            }
            running = true;
            ...
            new Thread(() -> {
                ...
                try {
                    completedReport = MemBandwidth.runMemoryBandwidthTest(
                            arrayMillions, iterations, littleThreads, bigThreads);
                } catch (Throwable error) {
                    completedReport = "ERROR: " + error.getMessage();
                }
                ...
                MAIN_HANDLER.post(() -> {
                    ComputeMemoryActivity activity = activeActivity.get();
                    if (activity != null) {
                        activity.renderBenchmarkState();
                    }
                });
            }, "membw-runner").start();
        }
