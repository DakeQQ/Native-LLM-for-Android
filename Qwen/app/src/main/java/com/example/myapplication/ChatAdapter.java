...
public class ChatAdapter extends RecyclerView.Adapter<ChatAdapter.ViewHolder> {
    private static final int VIDEO_FRAME_DELAY_MS = 500;
    private static final int MEDIA_CACHE_BYTES = 24 * 1024 * 1024;
    private static final Handler MAIN_HANDLER = new Handler(Looper.getMainLooper());
    private static final ExecutorService MEDIA_EXECUTOR = newSingleDaemonExecutor("chat-media");
    private static final ExecutorService MARKDOWN_EXECUTOR = newSingleDaemonExecutor("chat-markdown");
    private static final LruCache<String, Bitmap> MEDIA_CACHE =
            new LruCache<>(MEDIA_CACHE_BYTES) {
                @Override
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        int layoutRes = switch (viewType) {
            ...
            case ChatMessage.TYPE_USER_IMAGE, ChatMessage.TYPE_USER_VIDEO -> R.layout.item_user_image;
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        switch (message.type()) {
            ...
            case ChatMessage.TYPE_USER_IMAGE -> {
                stopVideoAnimation(holder);
                // User-side image bubble: show the captured vision snapshot thumbnail + optional status caption.
                if (holder.messageImage != null) {
                    bindImageFrame(holder, message, message.thumbnailPath());
                    holder.messageImage.setContentDescription(holder.itemView.getContext()
                            .getString(R.string.vision_image_bubble_desc));
                }
                bindVisionCaption(holder, message);
            }
            case ChatMessage.TYPE_USER_VIDEO -> {
                stopVideoAnimation(holder);
                if (holder.messageImage != null) {
                    holder.messageImage.setContentDescription(holder.itemView.getContext()
                            .getString(R.string.vision_video_bubble_desc));
                    List<String> frames = message.videoFramePaths();
                    if (holder.itemView.isAttachedToWindow()) {
                        bindVideoFrames(holder, message, frames);
                    } else if (frames != null && !frames.isEmpty()) {
                        bindImageFrame(holder, message, frames.get(0));
                    } else {
                        setDecodedBitmap(holder, null);
                    }
                }
                bindVisionCaption(holder, message);
            }
    ...
    private static void bindVisionCaption(ViewHolder holder, ChatMessage message) {
        if (holder.messageCaption == null) {
            return;
        }
        String content = message.content();
        if (isBlank(content) && !message.mediaPreviewFailed()) {
            holder.messageCaption.setText("");
            holder.messageCaption.setVisibility(View.GONE);
        } else {
            if (message.mediaPreviewFailed()) {
                String failure = holder.itemView.getContext().getString(R.string.media_preview_failed);
                holder.messageCaption.setText(isBlank(content) ? failure : content + "\n" + failure);
            } else {
                holder.messageCaption.setText(content);
            }
            holder.messageCaption.setVisibility(View.VISIBLE);
        }
    }

    private static void bindImageFrame(ViewHolder holder, ChatMessage message, String path) {
        setDecodedBitmap(holder, null);
        final int generation = holder.mediaGeneration;
        decodeFrameAsync(holder, message, path, generation, null);
    }

    private static void bindVideoFrames(ViewHolder holder, ChatMessage message, List<String> frames) {
        if (holder.messageImage == null || frames == null || frames.isEmpty()) {
            if (holder.messageImage != null) {
                setDecodedBitmap(holder, null);
            }
            return;
        ...
        setDecodedBitmap(holder, null);
        final int generation = holder.mediaGeneration;
        holder.videoFrameIndex = 0;
        holder.videoFrameRunnable = new Runnable() {
            @Override public void run() {
                if (holder.messageImage == null || holder.mediaGeneration != generation ||
                        holder.boundMessage != message) {
                    return;
                }
                holder.videoFrameIndex = (holder.videoFrameIndex + 1) % frames.size();
                decodeFrameAsync(holder, message, frames.get(holder.videoFrameIndex), generation,
                        frames.size() > 1 ? this : null);
            }
        };
        decodeFrameAsync(holder, message, frames.get(0), generation,
                frames.size() > 1 ? holder.videoFrameRunnable : null);
    }

    private static void decodeFrameAsync(ViewHolder holder, ChatMessage message, String path,
                                         int generation, Runnable nextFrame) {
        MEDIA_EXECUTOR.execute(() -> {
            Bitmap decoded = decodeBitmap(path);
            MAIN_HANDLER.post(() -> {
                if (holder.mediaGeneration != generation || holder.boundMessage != message ||
                        holder.messageImage == null) {
                    return;
                }
                setDecodedBitmap(holder, decoded);
                if (nextFrame != null) {
                    holder.messageImage.postDelayed(nextFrame, VIDEO_FRAME_DELAY_MS);
                }
            });
        });
    }

    private static void stopVideoAnimation(ViewHolder holder) {
        holder.mediaGeneration += 1;
        if (holder.messageImage != null && holder.videoFrameRunnable != null) {
            holder.messageImage.removeCallbacks(holder.videoFrameRunnable);
        }
        holder.videoFrameRunnable = null;
        holder.videoFrameIndex = 0;
    }
    ...
    private static Bitmap decodeBitmap(String path) {
        ...
        Bitmap cached = MEDIA_CACHE.get(path);
        if (cached != null && !cached.isRecycled()) {
            return cached;
        }
        Bitmap decoded = BitmapFactory.decodeFile(path);
        if (decoded != null) {
            MEDIA_CACHE.put(path, decoded);
        }
        return decoded;
    }

    private static void setDecodedBitmap(ViewHolder holder, Bitmap bitmap) {
        if (holder.messageImage != null) {
    ...
    void releaseVisibleVisionMediaFrom(RecyclerView recyclerView, int minAdapterPosition) {
        if (recyclerView == null) {
            return;
        }
        for (int i = 0; i < recyclerView.getChildCount(); i++) {
            RecyclerView.ViewHolder rawHolder = recyclerView.getChildViewHolder(recyclerView.getChildAt(i));
            if (!(rawHolder instanceof ViewHolder holder)) {
                continue;
            }
            int position = holder.getBindingAdapterPosition();
            if (position == RecyclerView.NO_POSITION || position < minAdapterPosition) {
                continue;
            }
            stopVideoAnimation(holder);
            if (holder.messageImage != null) {
    ...
    public void onViewAttachedToWindow(@NonNull ViewHolder holder) {
        ...
        ChatMessage message = holder.boundMessage;
        if (message != null && message.type() == ChatMessage.TYPE_USER_VIDEO) {
            stopVideoAnimation(holder);
            bindVideoFrames(holder, message, message.videoFramePaths());
        }
    }
    @Override
    ...
    static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        ImageView messageImage;   // present only on the user image bubble (item_user_image)
        TextView messageCaption;  // present only on the user image bubble (item_user_image)
