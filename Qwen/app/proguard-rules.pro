...
# glRender.cpp resolves this class, field and callback by their literal JNI names.
-keep class com.example.myapplication.CameraService {
    long nativeGLRender;
    native <methods>;
}

-keep interface com.example.myapplication.CameraService$InferenceFrameCallback {
    void onFrame(java.nio.ByteBuffer, java.nio.ByteBuffer, int, int);
}

-keepclassmembers class * implements com.example.myapplication.CameraService$InferenceFrameCallback {
    void onFrame(java.nio.ByteBuffer, java.nio.ByteBuffer, int, int);
}