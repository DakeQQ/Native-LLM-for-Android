...
class YuanAttention(nn.Module):
    ...
    def forward(
        ...
        value_states = torch.cat((past_value_states,
                                  self.v_proj(hidden_states).view(ids_len, self.num_heads,
        ...
        return self.o_proj(torch.matmul(nn.functional.softmax(
            ...
            dtype=torch.float32), value_states).transpose(0, 1).reshape(1, ids_len, self.hidden_size)), key_states, value_states, save_hidden_states
...
class YuanForCausalLM(YuanPreTrainedModel):
    ...
    def forward(
            self,
            hidden_states_B: torch.FloatTensor = None,
            attention_mask: Optional[torch.Tensor] = None,
        ...
        save_hidden = [None] * self.num_layers
        ...
        hidden_states_B = hidden_states_B[:, :ids_len, :]
        attention_mask = (1.0 - torch.tril(torch.ones([1, ids_len, kv_seq_len], dtype=torch.float32))) * attention_mask
        for i in range(self.num_layers):
            hidden_states_B, save_kv_0[i], save_kv_1[i], save_hidden[i] = self.model.layers[i](
