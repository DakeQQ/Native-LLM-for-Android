...
onnx_model_C = 'C:/Users/Downloads/Yuan_ONNX/Yuan_part_C.onnx'  # Assign a path where the exported Yuan2 part_C model stored.
...
modified_path_C = './modeling_modified_C/yuan_hf_model.py'  # The path where the modified part_C yuan_hf_model.py stored.
...
torch.onnx.export(
    ...
    output_names=['hidden_states_C', 'past_key_states', 'past_values_states', 'before_hidden_states'],
    do_constant_folding=True,
    opset_version=17)
del model
...
shutil.copyfile(modified_path_C, path + "/yuan_hf_model.py")
model = AutoModelForCausalLM.from_pretrained(path, torch_dtype=torch.float32, device_map='cpu', trust_remote_code=True).float().eval()
torch.onnx.export(
    model, hidden_states_C,
    onnx_model_C,
    input_names=['hidden_states_C'],
...
in_name_B8 = in_name_B[8].name
in_name_B9 = in_name_B[9].name
...
past_key_states_B = np.zeros((num_layers, num_heads, max_seq_len, head_dim), dtype=np.float32)
...
while history_len < max_single_chat_length:
    ...
    hidden_states_C, past_key_states_B, past_values_states_B, before_hidden_states = ort_session_B.run(
        [out_name_B0, out_name_B1, out_name_B2, out_name_B3],
        {in_name_B0: hidden_states_B,
         ...
         in_name_B7: ids_len,
         in_name_B8: before_hidden_states,
         in_name_B9: control_factor})
