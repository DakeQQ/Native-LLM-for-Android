...
class GemmaSdpaAttention(GemmaAttention):
    ...
    def forward(
        ...
        return self.o_proj(torch.matmul(nn.functional.dropout(nn.functional.softmax(
            ...
            value_states).transpose(0, 1).contiguous().reshape(ids_len, self.hidden_size)), save_key_states, save_value_states
...
class GemmaForCausalLM(GemmaPreTrainedModel):
    ...
    def forward(
        ...
        hidden_states = self.model.embed_tokens(input_ids[:, :ids_len]) * self.scale_emb
        ...
        expand_space = torch.zeros((self.num_layers, self.num_key_value_heads, self.max_seq_len - kv_seq_len, self.head_dim),
                                   dtype=torch.float32)
