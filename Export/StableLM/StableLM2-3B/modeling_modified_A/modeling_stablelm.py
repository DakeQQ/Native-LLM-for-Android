...
class StableLmAttention(nn.Module):
    ...
    def forward(
        ...
        key_states = torch.cat((past_key_states, torch.cat((key_states * rotary_pos_emb_cos + rotate_half(key_states) * rotary_pos_emb_sin, key_pass), dim=-1)), dim=-2)
        ...
        return self.o_proj(torch.matmul(nn.functional.softmax(
            torch.matmul(torch.cat((query_states * rotary_pos_emb_cos + rotate_half(query_states) * rotary_pos_emb_sin, query_pass), dim=-1),
                         key_states.transpose(1, 2)) * self.head_dim_factor + attention_mask, dim=-1,
            dtype=torch.float32), value_states).transpose(0, 1).contiguous().reshape(1, ids_len, self.hidden_size)), key_states, value_states
...
class StableLmSdpaAttention(StableLmAttention):
    def forward(
        ...
        key_states = torch.cat((past_key_states, torch.cat(
            (key_states * rotary_pos_emb_cos + rotate_half(key_states) * rotary_pos_emb_sin, key_pass), dim=-1)), dim=-2)
        value_states = torch.cat((past_value_states, self.v_proj(hidden_states).view(ids_len, self.num_key_value_heads, self.head_dim).transpose(0, 1)), dim=-2)
        return self.o_proj(torch.matmul(nn.functional.softmax(
            torch.matmul(
                torch.cat((query_states * rotary_pos_emb_cos + rotate_half(query_states) * rotary_pos_emb_sin, query_pass), dim=-1),
                key_states.transpose(1, 2)) * self.head_dim_factor + attention_mask, dim=-1,
            dtype=torch.float32), value_states).transpose(0, 1).contiguous().reshape(1, ids_len, self.hidden_size)), key_states, value_states
...
class StableLmForCausalLM(StableLmPreTrainedModel):
    ...
    def forward(
        ...
        for i in range(self.num_layers):
            hidden_states, save_kv_0[i], save_kv_1[i] = self.model.layers[i](
        ...
        expand_space = torch.zeros(
            (self.num_layers, self.num_key_value_heads, self.max_seq_len - kv_seq_len, self.head_dim),
            dtype=torch.float32)
        return (torch.cat((hidden_states, torch.zeros((1, self.max_seq_len - ids_len, self.hidden_size), dtype=torch.float32)), dim=-2),
