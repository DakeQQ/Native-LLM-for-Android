...
class PhiAttention(nn.Module):
    ...
    def forward(
        ...
        return self.dense(torch.matmul(nn.functional.softmax(torch.matmul(
            torch.cat((query_states * rotary_pos_emb_cos + rotate_half(query_states) * rotary_pos_emb_sin, query_pass), dim=-1), key_states.transpose(1, 2)) * self.head_dim_factor + attention_mask, dim=-1, dtype=torch.float32).to(torch.float32),
                                       value_states).transpose(0, 1).contiguous().reshape(1, ids_len, self.hidden_size)), key_states, value_states
...
class PhiDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        attn_outputs, past_key_states, past_value_states = self.self_attn(
        ...
        return (self.resid_dropout(attn_outputs) + self.resid_dropout(self.mlp(hidden_states)) + residual,) + (past_key_states,) + (past_value_states,)
...
class PhiForCausalLM(PhiPreTrainedModel):
    ...
    def forward(
        ...
        hidden_states = self.model.embed_dropout(self.model.embed_tokens(input_ids[:, :ids_len]))
