...
download_path = r'/home/DakeQQ/Downloads/Qwen3-VL-2B-Instruct'                 # Set the folder path where the LLM whole project downloaded, otherwise set "NONE".
...
for model_name in model_names:
    ...
    if ('QwenVL_C' in model_name) or ('QwenVL_D' in model_name) or ('QwenVL_E' in model_name):
        use_low_memory_mode_current = False
    else:
        use_low_memory_mode_current = use_low_memory_mode_in_Android
    ...
    if model_size_gb > 2.0:
        ...
        is_large_model = True if use_low_memory_mode_current else False
        ...
        try:
            if ('vl' in download_path_lower) & ('qwen' in download_path_lower):
                if "2.5" in download_path:
                    ...
                    model_config = Qwen2_5_VLForConditionalGeneration.from_pretrained(download_path, torch_dtype=torch.float16, device_map='cpu', trust_remote_code=True, low_cpu_mem_usage=True).eval()
                elif "3" in download_path:
                    from transformers import Qwen3VLForConditionalGeneration
                    model_config = Qwen3VLForConditionalGeneration.from_pretrained(download_path, torch_dtype=torch.float16, device_map='cpu', trust_remote_code=True, low_cpu_mem_usage=True).eval()
                else:
                    from transformers import Qwen2VLForConditionalGeneration
                    model_config = Qwen2VLForConditionalGeneration.from_pretrained(download_path, torch_dtype=torch.float16, device_map='cpu', trust_remote_code=True, low_cpu_mem_usage=True).eval()
            else:
                if ("gemma3" in download_path_lower) or ("gemma 3" in download_path_lower) or ("gemma-3" in download_path_lower):
                    from transformers import Gemma3ForCausalLM
                    model_config = Gemma3ForCausalLM.from_pretrained(download_path, torch_dtype=torch.float16, device_map='cpu', trust_remote_code=True, low_cpu_mem_usage=True).eval()
                else:
                    model_config = AutoModelForCausalLM.from_pretrained(download_path, torch_dtype=torch.float16, device_map='cpu', trust_remote_code=True, low_cpu_mem_usage=True).eval()
    ...
    # Convert to ORT format if not using low memory mode
    if not is_large_model:
        ...
        subprocess.run(
            f'python -m onnxruntime.tools.convert_onnx_models_to_ort --output_dir {quanted_folder_path} --optimization_style {optimization_style} --target_platform {target_platform} --enable_type_reduction {quanted_model_path}',
            shell=True
        )
