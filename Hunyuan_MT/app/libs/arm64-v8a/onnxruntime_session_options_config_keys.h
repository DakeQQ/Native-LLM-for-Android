...
// Enables lookup-before-pack for kernels that can derive a complete deterministic source key.
// "0": Disabled (default). Preserves cold session-creation latency.
// "1": Enabled. Can reduce session creation when a PrepackedWeightsContainer is reused across sessions,
//      at the cost of additional source-key work when the container has no matching entry.
static const char* const kOrtSessionOptionsEnablePrepackedWeightsSourceAlias =
    "session.enable_prepacked_source_alias";
