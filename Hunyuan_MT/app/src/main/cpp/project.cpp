...
//        }
    ...
    if (chatting) {  // Java multithreading may not stop immediately. Therefore, use a switch to prevent over runs.
        ...
        if ((token_id != end_id_0) && (response_count < single_chat_limit) && (history_len < max_seq_len)) {
            for (int i = 0; i < amount_of_output; i++) {
                                                            ...
                                                            jboolean low_memory_mode)
{
    ...
    OrtSessionOptions *session_options_A;
    {
        ...
        ort_runtime_A->AddSessionConfigEntry(session_options_A, "optimization.graph_optimizations_loop_level",
                                             "2");
