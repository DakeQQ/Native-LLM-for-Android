...
#include <cstring>
#include <utility>
#include "unicode.hpp"   // MNN::Unicode::regex_scanner -- Unicode-aware pre-tokenizer for the byte-level BPE encode

namespace MNN::Transformer {

#define TOKENIZER_LOGE(...) __android_log_print(ANDROID_LOG_ERROR, "Tokenizer", __VA_ARGS__)

// base64
// Decode table covering all 256 byte values (any invalid byte, including everything >= 128, maps to
// -1). int8_t keeps the whole table in a single 256-byte block for a branchless cache-friendly probe.
static const int8_t kBase64DecodeTable[256] = {
...
static std::string base64_decode(const std::string& str) {
    if (str.empty()) return "";
    auto in_len = static_cast<uint32_t>(str.size());
    std::string ret;
...
static inline uint32_t one_char_len(const char *src) {
...
Tokenizer* Tokenizer::createTokenizer(const std::string& filename) {
    ...
    if (!tok_file.good()) {
        TOKENIZER_LOGE("Failed to load tokenizer from: %s", filename.c_str());
        return tokenizer;
    }
    ...
    if (magic_number != MAGIC_NUMBER) {
        TOKENIZER_LOGE("Invalid tokenizer magic number in: %s", filename.c_str());
        return tokenizer;
    }
...
void Tokenizer::load_special(std::ifstream& tok_file) {
    ...
    if (stop_num) {
        for (int i = 0; i < stop_num; i++) {
            int ignoredStopToken;
            specail_line >> ignoredStopToken;
        }
    }
    if (prefix_num) {
        // load prefix tokens
...
void Tokenizer::cache_special_tokens() {
    ...
    special_tokens_cache_.reserve(special_tokens_.size());
    for (auto& bucket : special_first_byte_buckets_) {
        bucket.clear();
    }
    for (int id : special_tokens_) {
        std::string token_str = decode(id);
        if (!token_str.empty()) {
            special_tokens_cache_.emplace_back(std::move(token_str), id);
        }
    }
    // Index every cached special token by its first byte, preserving cache order within each bucket so
    // encode() reproduces the original "first match in cache order wins" behavior exactly.
    for (const auto& entry : special_tokens_cache_) {
        auto first_byte = static_cast<unsigned char>(entry.first[0]);
        special_first_byte_buckets_[first_byte].push_back(entry);
    }
}

std::vector<int> Tokenizer::encode(const std::string& str) {
    return encode(str, std::numeric_limits<size_t>::max());
}

std::vector<int> Tokenizer::encode(const std::string& str, size_t max_tokens) {
    std::vector<int> ids;
    ids.reserve(std::min(prefix_tokens_.size(), max_tokens));
    ids.insert(ids.end(), prefix_tokens_.begin(),
               prefix_tokens_.begin() + static_cast<std::ptrdiff_t>(
                       std::min(prefix_tokens_.size(), max_tokens)));
    ...
    if (!special_tokens_cache_.empty()) {
        // Scan once over the raw input (no copy). At each position only special tokens that share the
        // current byte as their first byte can possibly match, so a 256-way first-byte bucket skips
        // the vast majority of positions in O(1) instead of rescanning every special token per char.
        const char* data = str.data();
        const auto n = static_cast<uint32_t>(str.size());
        std::string segment;   // reused across splits: assign() keeps capacity, no per-segment alloc
        uint32_t start = 0;
        for (uint32_t i = 0; i < n; ++i) {
            const auto& bucket = special_first_byte_buckets_[static_cast<unsigned char>(data[i])];
            if (bucket.empty()) {
                continue;
            }
            for (const auto& pair : bucket) {
                const std::string& token = pair.first;
                const auto tlen = static_cast<uint32_t>(token.size());
                if (i + tlen <= n && std::memcmp(data + i, token.data(), tlen) == 0) {
                    if (i > start) {
                        segment.assign(data + start, i - start);
                        if (!encode_limited(segment, ids, max_tokens) ||
                            ids.size() >= max_tokens) {
                            return ids;
                        }
                    }
        ...
        if (start < n) {
            if (start == 0) {
                encode_limited(str, ids, max_tokens);   // no special matched anywhere: no copy
            } else {
                segment.assign(data + start, n - start);
                encode_limited(segment, ids, max_tokens);
            }
        }
    } else {
        encode_limited(str, ids, max_tokens);
    }
    return ids;
}
...
Sentencepiece::EncodeResult Sentencepiece::bpe_encode(string_view_ normalized, float alpha) {
    ...
    struct SymbolPair {
        ...
        uint32_t size;  // length of this piece
    };
...
void Sentencepiece::encode(const std::string& str, std::vector<int>& ids) {
    // For SentencePiece, replace all spaces with ▁ (U+2581) and add a ▁ prefix to the beginning.
    std::string normalized_str = "\xe2\x96\x81";
    normalized_str.reserve(str.size() + 16);   // pre-size; ▁ is 3 bytes, most chars copy 1:1
    for (char c : str) {
        if (c == ' ') {
            normalized_str += "\xe2\x96\x81";
        } else {
            normalized_str += c;
        }
    }
...
std::string Sentencepiece::decode(int id) {
    ...
    const std::string& piece = sentence_pieces_[id].piece;
    // ▁ (U+2581, the SentencePiece space marker) is the 3 bytes 0xE2 0x96 0x81.
    size_t pos = piece.find("\xe2\x96\x81");
    if (pos == std::string::npos) {
        return piece;                      // common case: no marker, no rewrite/extra copy
    }
    std::string out;
    out.reserve(piece.size() - 2);         // the 3 marker bytes collapse to a single space
    out.append(piece, 0, pos);
    out += ' ';
    out.append(piece, pos + 3, std::string::npos);
    return out;
}
...
bool Tiktoken::load_vocab(std::ifstream& tok_file) {
    ...
    std::getline(tok_file, line);
    int vocab_len = std::stoi(line);
    // Decode every vocab token first. The trie node count can never exceed the total number of
    // token bytes (each node is a distinct prefix), so reserving that exact upper bound up front
    // means the build never reallocates the node pool (peak stays below the old power-of-two growth).
    decoder_.resize(vocab_len);
    for (int i = 0; i < vocab_len; i++) {
        std::getline(tok_file, line);
        decoder_[i] = base64_decode(line);
    }
    uint32_t node_upper_bound = 1; // root
    for (const std::string& tok : decoder_) { node_upper_bound += static_cast<uint32_t>(tok.size()); }
    encoder_.reserve(static_cast<int>(node_upper_bound));
    for (int i = 0; i < vocab_len; i++) {
        encoder_.insert({decoder_[i], i}); // Trie copies the bytes node-by-node during insert.
    }
    // Compact the trie into the read-optimized CSR layout and free the build-time structures.
    encoder_.finalize();
    return true;
}

void Tiktoken::encode(const std::string& str, std::vector<int>& ids) {
    encode_limited(str, ids, std::numeric_limits<size_t>::max());
}

bool Tiktoken::encode_limited(const std::string& str, std::vector<int>& ids,
                              size_t max_tokens) {
    if (str.empty()) {
        return true;
    }
    // Greedy longest-match over the trie. Reserve assuming a few bytes per token so long prompts do
    // not trigger repeated reallocations of the output id vector.
    const size_t estimate = str.size() / 3 + 1;
    const size_t available = max_tokens > ids.size() ? max_tokens - ids.size() : 0;
    ids.reserve(ids.size() + std::min(estimate, available));
    const auto end = str.end();
    auto it = str.begin();
    while (it != end) {
        int token_id = encoder_.find(it, end);   // advances `it` past the matched token (or by 1)
        if (token_id >= 0) {
            if (ids.size() >= max_tokens) {
                return false;
            }
...
bool BertTokenizer::load_vocab(std::ifstream& tok_file) {
    ...
    // Resolve the [UNK] id once from the vocab (-1 if absent) so word_piece never emits a hard-coded magic
    // id that could map to an unrelated token in a vocab where [UNK] != 100.
    auto unk_it = encoder_.find("[UNK]");
    unk_id_ = (unk_it != encoder_.end()) ? unk_it->second : -1;
    return true;
}
...
std::vector<int> BertTokenizer::word_piece(const std::string& token) {
    ...
    while (!current.empty()) {
        ...
        for (auto len = static_cast<uint32_t>(current.size()); len > 0; --len) {
            candidate.clear();
        ...
        // If no match found, emit the vocab's [UNK] id (resolved once in load_vocab). If the vocab has no
        // [UNK] (unk_id_ < 0) emit nothing rather than a guessed id that could decode to junk.
        if (match_id == -1) {
            if (unk_id_ >= 0) {
...
std::wstring utf8_to_wstring(const char* str, uint32_t len) {
...
std::string wstring_to_utf8(const std::wstring& str) {
    ...
    while (p < end) {
        auto cp = static_cast<unsigned int>(*p);
        p++;
        if (sizeof(wchar_t) == 2) {
            if (cp >= 0xD800 && cp <= 0xDBFF) {
                if (p < end) {
                    auto low = static_cast<unsigned int>(*p);
...
bool HuggingfaceTokenizer::load_vocab(std::ifstream& tok_file) {
    ...
    // 4. bytes_to_unicode initialization (flat arrays instead of hash maps)
    for (int i = 0; i < 512; ++i) u2b_[i] = -1;
    wchar_t temp_map[256] = {};
    ...
    int n = 0;
    for (wchar_t & b : temp_map) {
        if (b == 0) {
            b = static_cast<wchar_t>(256 + n);
            n++;
        }
    }

    // Fill the byte<->unicode arrays directly (every byte maps to a code point < 0x144).
    for (int i = 0; i < 256; ++i) {
        wchar_t wc = temp_map[i];
        b2u_[i] = wc;
        if (wc >= 0 && wc < 512) u2b_[wc] = static_cast<int16_t>(i);
    }
...
void HuggingfaceTokenizer::bpe(const std::wstring& token, const BPERanks& bpe_ranks, std::vector<std::wstring>* result) {
    ...
    // `merged_flag[i]` marks pair i as merged. A flat byte array + counter replaces the former
    // std::set<int> (O(1) membership, no tree-node allocations). Conceptual index -1 is never merged.
    std::vector<uint8_t> merged_flag(num_pairs, 0);
    int merged_count = 0;
    auto _left = [&](int i) {
        for (int j = i - 1; j >= 0; j--) {
            if (!merged_flag[j]) return j;
        }
        return -1;
    };
    auto _right = [&](int i, int cap) {
    ...
    while (true) {
        ...
        int to_merge = -1;  // index into pairs.

        for (int i = 0; i < num_pairs; ++i) {
            if (!merged_flag[i]) {  // pair i is not merged.
        ...
        merged_flag[to_merge] = 1;
        ++merged_count;
        std::wstring merge_into = pairs[to_merge].first + pairs[to_merge].second;

        int l = _left(to_merge);
        if (l >= 0) pairs[l].second = merge_into;
        int r = _right(to_merge, num_pairs);
        if (r < num_pairs) pairs[r].first = merge_into;
    }  // end while (true)

    if (merged_count == num_pairs) {
        result->push_back(token);

    } else {
        for (int i = 0; i < num_pairs; ++i) {
            if (!merged_flag[i]) {
                if (_left(i) < 0) result->push_back(pairs[i].first);
...
void HuggingfaceTokenizer::encode(const std::string& str, std::vector<int>& ids) {
    // Qwen pre-tokenizer regex (tokenizer.json). It MUST be Unicode-aware: the previous std::regex used
    // ASCII-only POSIX classes ([[:alpha:]]/[[:digit:]]) that do NOT match Unicode letters/digits, so CJK
    // text + adjacent punctuation collapsed into one pre-token instead of splitting on \p{L}/\p{N} bounda-
    // ries -- byte-level BPE could then merge across a boundary the reference keeps, yielding different
    // token ids for Chinese/mixed-script input. MNN::Unicode::regex_scanner folds every codepoint to a
    // category byte and runs a byte-regex that honours \p{L}, \p{N}, \s and (?i:)/(?!\S), reproducing the
    // canonical Qwen split (same engine the PipelineTokenizer uses).
    static const std::string kQwenPreTokenizePattern =
        R"((?i:'s|'t|'re|'ve|'m|'ll|'d)|[^\r\n\p{L}\p{N}]?\p{L}+|\p{N}| ?[^\s\p{L}\p{N}]+[\r\n]*|\s*[\r\n]+|\s+(?!\S)|\s+)";

    std::wstring wtoken;
    std::vector<std::wstring> bpe_tokens;
    // One O(n) pass over the raw UTF-8 input; wtoken/bpe_tokens are reused across pre-tokens.
    const std::vector<std::string> pre_tokens = Unicode::regex_scanner(str, kQwenPreTokenizePattern);
    for (const std::string& token : pre_tokens) {
        if (token.empty()) {
            continue;
        }
        wtoken.clear();
        ...
        for (char c : token) {
            wtoken.push_back(b2u_[uint8_t(c)]);
        }
        ...
        for (const auto& ws : bpe_tokens) {
            const std::string piece = wstring_to_utf8(ws);
            ...
            for (wchar_t wc : ws) {
                const std::string fallback_piece = wstring_to_utf8(std::wstring(1, wc));
                auto fit = encoder_.find(fallback_piece);
                if (fit != encoder_.end()) {
...
std::string HuggingfaceTokenizer::decode(int id) {
    ...
    // Decode UTF-8 and reverse the byte-level alphabet in one pass with one bounded reservation.
...
public:
    ...
    std::string normalize(const std::string& text) const override {
        ...
        const auto* ptr = (const uint8_t*)text.data();
        uint32_t len = (uint32_t)text.size(), i = 0;
        while (i < len) {
            int32_t cp;
            ...
            if (cp != 0 && cp != '\n' && cp != '\r' && cp != '\t' &&
                ...
                uint32_t lo = 0, hi = (uint32_t)table_.size();
...
class PrependNormalizer : public Normalizer {
    std::string prepend_;
public:
    PrependNormalizer(std::string  p) : prepend_(std::move(p)) {}
...
public:
    ReplaceNormalizer(std::string  p, const std::string& c) : pattern_(std::move(p)), content_(c) {}
    std::string normalize(const std::string& text) const override {
...
public:
    ...
    std::string normalize(const std::string& text) const override {
        std::string out;
        const auto* ptr = (const uint8_t*)text.c_str();
        uint32_t len = (uint32_t)text.length(), i = 0;
        int32_t cp;
        while (i < len) {
            int r = Unicode::utf8_decode(ptr + i, len - i, &cp);
            if (r <= 0) { i++; continue; }
            ...
            if (strip_accents_ && !nfd_table_.empty()) {
                ...
                const auto* dptr = (const uint8_t*)decomposed.data();
                uint32_t dlen = (uint32_t)decomposed.size(), dj = 0;
                while (dj < dlen) {
                    int32_t dcp;
...
private:
    std::string nfd_lookup(const std::string& ch) const {
        const auto* ptr = (const uint8_t*)ch.data();
        uint32_t len = (uint32_t)ch.size(), i = 0;
        std::string out;
        out.reserve(len * 2);
        while (i < len) {
            int32_t cp;
            int r = Unicode::utf8_decode(ptr + i, len - i, &cp);
            uint32_t lo = 0, hi = (uint32_t)nfd_table_.size();
            const std::string* found = nullptr;
            while (lo < hi) {
                uint32_t mid = (lo + hi) / 2;
...
public:
    ByteLevelPreTokenizer(bool use_regex = false) : use_regex_(use_regex),
        gpt2_pattern_(R"('s|'t|'re|'ve|'m|'ll|'d| ?\p{L}+| ?\p{N}+| ?[^\s\p{L}\p{N}]+|\s+(?!\S)|\s+)") {}
    void pre_tokenize(PreTokenizedString& pts) const override {
        if (use_regex_) {
...
public:
    ...
    MetaspacePreTokenizer(std::string  rep, bool aps) : replacement_(std::move(rep)), add_prefix_space_(aps) {}
    void pre_tokenize(PreTokenizedString& pts) const override {
        for (auto& s : pts.splits) {
...
public:
    ...
    SplitPreTokenizer(std::string  pattern, bool invert, const std::string& behavior = "Isolated")
        : pattern_(std::move(pattern)), invert_(invert), behavior_(behavior) {}
...
public:
    void pre_tokenize(PreTokenizedString& pts) const override {
        ...
        for (const auto& s : pts.splits) {
            std::string current;
            const auto* ptr = (const uint8_t*)s.c_str();
            uint32_t len = (uint32_t)s.length(), i = 0;
            int32_t cp;
            while (i < len) {
                int r = Unicode::utf8_decode(ptr + i, len - i, &cp);
                if (r <= 0) { i++; continue; }
...
public:
    ...
    std::vector<int> tokenize(const std::string& text) const override {
        ...
        if (use_byte_level_) {
            ...
            const auto* ptr = (const uint8_t*)text.c_str();
            uint32_t len = (uint32_t)text.length(), off = 0;
            int32_t cp;
            while (off < len) {
...
public:
    ...
    std::vector<int> tokenize(const std::string& text) const override {
        ...
        uint32_t n = (uint32_t)text.length();
        std::vector<double> best_scores(n + 1, -1e18);
        std::vector<int> best_ids(n + 1, -1);
        std::vector<uint32_t> best_prev_pos(n + 1, 0);

        best_scores[0] = 0.0;

        for (uint32_t i = 1; i <= n; ++i) {
            uint32_t start_len = (i > max_token_len_) ? (i - max_token_len_) : 0;
            for (uint32_t j = i - 1; j != (uint32_t)-1 && j >= start_len; --j) {
...
void PipelineTokenizer::encode(const std::string& str, std::vector<int>& ids) {
    ...
    while (last < str.length()) {
        if (added_token_strings_.empty()) {
            units.emplace_back(str.substr(last), false);
            break;
        }
        ...
        if (match_pos >= 0 && match_idx >= 0) {
            ...
            uint32_t prefix_start = last;
            auto prefix_end = (uint32_t)match_pos;
            uint32_t next_start = (uint32_t)match_pos + (uint32_t)at.content.size();
            ...
            if (prefix_end > prefix_start) units.emplace_back(str.substr(prefix_start, prefix_end - prefix_start), false);
            units.emplace_back(at.content, true);
            last = next_start;
        } else {
            units.emplace_back(str.substr(last), false);
            break;
...
std::string PipelineTokenizer::decode(int id) {
    ...
    if (byte_level_) {
        ...
        std::string out;
        const auto* p = (const uint8_t*)result.data();
        uint32_t len = (uint32_t)result.size(), i = 0;
        while (i < len) {
...
bool PipelineTokenizer::load_vocab(std::ifstream&) { return false; }
...
bool PipelineTokenizer::load_vocab_binary(std::ifstream& file) {
    ...
    auto remaining = (size_t)(end_pos - cur_pos);
    file.seekg(cur_pos);
    ...
    // --- Model --- (zero-copy: StringRef points into buf)
    {
        ...
        if (type == 0) { // BPE
            ...
            for (uint32_t i = 0; i < merge_size; i++) {
                ...
                merges.emplace_back(merge_key((int)id1, (int)id2), (int)rank);
            }
    ...
    // Streaming decode needs only byte-level and WordPiece metadata; consume the rest without objects.
    std::function<bool(const char*&)> skip_decoder_bin;
    skip_decoder_bin = [&](const char*& p) -> bool {
        uint8_t type = read_u8(p);
        switch (type) {
            case 0:
                byte_level_ = true;
            ...
            case 2:
                ...
                return true;
            case 3:
                wordpiece_prefix_ = read_str(p);
                read_u8(p);
                wordpiece_decode_ = true;
                return true;
            case 5:
                read_str(p);
                read_str(p);
    ...
    // The runtime builds chat tokens itself; skip optional template strings without allocating them.
    ...
    // Full-sequence cleanup flag is retained in the format but unused by streaming decode.
    if (ptr < buf_end) {
        read_u8(ptr);
    }

    // Skip the optional BOS string; special-token IDs are loaded separately above.
    if (ptr < buf_end) {
