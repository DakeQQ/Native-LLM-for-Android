...
void HuggingfaceTokenizer::encode(const std::string& str, std::vector<int>& ids) {
    ...
    // Iterate matches over `str` directly. The previous loop did `input = match.suffix().str()` each
    // iteration, copying the entire remaining string every time (O(n^2)); sregex_iterator walks the
    // same non-overlapping matches in O(n). wtoken/bpe_tokens are reused across iterations.
    auto rit = std::sregex_iterator(str.begin(), str.end(), re);
    auto rend = std::sregex_iterator();
    for (; rit != rend; ++rit) {
        const std::string& token = rit->str();
        wtoken.clear();
        ...
        for (const auto& ws : bpe_tokens) {
            auto eit = encoder_.find(wstring_to_utf8(ws));
            if (eit != encoder_.end()) {
