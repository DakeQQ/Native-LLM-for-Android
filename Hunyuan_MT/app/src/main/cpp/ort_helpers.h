...
// Case-insensitive Qwen-style tokenizer asset name: vocab_*.txt. The file header, not its extension,
// selects the concrete tokenizer implementation (including the binary PipelineTokenizer format).
...
// First existing tokenizer file in `dir`, or "" if the dir is missing / has none.
...
// Resolve the tokenizer path. Prefer the configured path, else auto-discover vocab_*.txt in
// cache/storage. Falls back to vocab_path so createTokenizer fails loudly.
...
// ── Shared initializers ────────────────────────────────────────────────────────────────────────
// The merged text strategy graphs (LLM_Text{Prefill,Decode}{Greedy,...}) all reference ONE
...
inline bool ortLoadModelSession(ModelRuntime& m, const char* onnxName, const char* externalName,
    ...
    // graphs plus the tiny per-token KV/RoPE control graphs. Prefill strategy graphs stay on the wide pool.
