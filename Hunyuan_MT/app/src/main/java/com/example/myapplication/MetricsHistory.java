...
public final class MetricsHistory {
    ...
    private boolean prefillDirty;
    private boolean decodeDirty;
    ...
    /**
     * Binds the app context synchronously (so an early writer can still persist), then loads the saved
     * throughput points on a background daemon thread. Keeps {@link App#onCreate()} off the disk on the
     * main thread; the read finishes well before any generation adds live points.
     */
    public void restoreAsync(Context context) {
        final Context resolved = context.getApplicationContext();
        synchronized (this) {
            appContext = resolved;
        }
        Thread loader = new Thread(() -> restore(resolved), "metrics-restore");
        loader.setDaemon(true);
        loader.start();
    }

    /** Writes dirty throughput rings to {@link App#PREFS}. No-op before {@link #restore(Context)}. */
    private void persistThroughput() {
        if (appContext == null || (!prefillDirty && !decodeDirty)) {
            return;
        }
        SharedPreferences prefs = appContext.getSharedPreferences(App.PREFS, Context.MODE_PRIVATE);
    ...
    /** Flushes live decode samples once when a generation ends. */
    public synchronized void flushThroughput() {
        persistThroughput();
    }
    ...
    public synchronized void clear() {
        ...
        prefillDirty = true;
        decodeDirty = true;
        persistThroughput();   // wipe the persisted copy so it does not reload on restart
    }
