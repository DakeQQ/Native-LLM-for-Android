...
{
    tokenizer = MNN::Transformer::Tokenizer::createTokenizer(cache_path + "vocab_MiniCPM.txt");
    return JNI_TRUE;
}
...
Java_com_example_myapplication_MainActivity_Run_1LLM(JNIEnv *env, jclass clazz, jstring jquery,
    ...
    if (add_prompt) {
        ...
        get_ids.erase(get_ids.begin());
        get_ids.insert(get_ids.begin(), {1, 73441, 3060, 5});                // MniCPM Chat prompt head
        get_ids.insert(get_ids.end(), {73440, 59320, 5, 73441, 16434, 5});   // MniCPM Chat prompt tail
                                                            ...
                                                            jboolean low_memory_mode)
{
    ...
    OrtSessionOptions *session_options_A;
    {
        ...
        ort_runtime_A->SetInterOpNumThreads(session_options_A, 8);
