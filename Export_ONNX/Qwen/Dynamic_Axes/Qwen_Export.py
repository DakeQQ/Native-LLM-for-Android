...
past_key_states = torch.zeros((num_layers, num_key_value_heads, 0, head_dim), dtype=torch.float16)
...
with torch.inference_mode():
    torch.onnx.export(
        ...
        input_names=[
            ...
            'attention_mask',
            'in_past_key_states',
            'in_past_values_states'
        ],
        output_names=['max_logit_id', 'out_past_key_states', 'out_past_values_states'],
        dynamic_axes={
            'input_ids': {1: 'ids_len'},
            'in_past_key_states': {2: 'history_len'},
            'in_past_values_states': {2: 'history_len'},
            'out_past_key_states': {2: 'history_len_plus_ids_len'},
            'out_past_values_states': {2: 'history_len_plus_ids_len'}
        },
...
past_key_states_A = np.zeros((num_layers, num_key_value_heads, 0, head_dim), dtype=np.float16)
...
while num_decode < max_single_chat_length:
    input_ids, past_key_states_A, past_values_states_A = ort_session_A.run(
    ...
    if input_ids in [151643, 151645]:  # the stop_id in Qwen is "151643" & "151645"
        break
    else:
        if num_decode < 1:
