...
class QWEN(torch.nn.Module):
    def __init__(self, qwen, max_seq_len, num_heads, num_key_value_heads, head_dim, num_layers):
    ...
    def forward(self, *all_inputs):
        ...
        for i, layer in enumerate(self.qwen.model.layers):
            ...
            q = layer.self_attn.q_norm(q).transpose(0, 1)
            k = layer.self_attn.k_norm(k).permute(2, 1, 3, 0)
        ...
        return *self.save_key, *self.save_value, torch.argmax(self.qwen.lm_head(self.qwen.model.norm(hidden_states[:, -1])), dim=-1, keepdim=True).int()
...
with torch.inference_mode():
    ...
    model = QWEN(model, MAX_SEQ_LEN, num_heads, num_key_value_heads, head_dim, num_layers)
