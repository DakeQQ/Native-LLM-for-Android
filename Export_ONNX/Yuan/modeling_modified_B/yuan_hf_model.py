...
class LocalizedFiltering(torch.nn.Module):
    ...
    def __init__(self, hidden_size):
        ...
        self.conv1 = torch.nn.Conv2d(self.embed_dim, self.embed_dim // 2, (2, 1), stride=(1, 1),
                                     padding=(self.lf_conv2d_num_pad, 0), groups=self.lf_conv2d_group)
        self.conv2 = torch.nn.Conv2d(self.embed_dim // 2, self.embed_dim, (2, 1), stride=(1, 1),
                                     padding=(self.lf_conv2d_num_pad, 0), groups=self.lf_conv2d_group)
    ...
    def _inference_forward(self, inputs, before_hidden_states, seq_len, control_factor):
        residual = inputs.transpose(0, 1)
        return self.output_layernorm(self.conv2(self.conv1(
            torch.cat((before_hidden_states, inputs), dim=1).transpose(0, 1).view(seq_len, 1, 1,
                    self.embed_dim).permute(2, 3, 0, 1))[:, :, control_factor:seq_len + control_factor, :])[:, :, control_factor:seq_len - control_factor, :].permute(2, 3, 0, 1).contiguous().view(seq_len - control_factor - control_factor, 1, self.embed_dim) + residual).transpose(0, 1)
    ...
    def forward(
        ...
        return self._inference_forward(inputs, before_hidden_states, ids_len, control_factor)
...
class YuanRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        return self.weight * hidden_states * torch.rsqrt(hidden_states.pow(2).mean(-1, keepdim=True) + self.variance_epsilon)
...
class YuanAttention(nn.Module):
    ...
    def __init__(self, config: YuanConfig):
        ...
        self.num_heads_2 = self.head_dim + self.head_dim
    ...
    def forward(
        ...
        temp_factor = before_hidden_states.shape[1] * control_factor
        hidden_states = self.lf_gate(hidden_states[:, :ids_len, :], before_hidden_states[:, :temp_factor, :], ids_len + temp_factor, control_factor)
        ...
        return self.o_proj(torch.matmul(nn.functional.softmax(
            torch.matmul((query_states * rotary_pos_emb_cos) + (rotate_half(query_states) * rotary_pos_emb_sin), key_states.transpose(1, 2).float()) * self.softmax_scale + attention_mask, dim=-1,
            dtype=torch.float32), value_states.float()).transpose(0, 1).reshape(1, ids_len, self.hidden_size)), key_states, value_states, save_hidden_states.half()
...
class YuanDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states_temp, past_key_states, past_value_states, save_hidden_states = self.self_attn(
            ...
            ids_len=ids_len,
            before_hidden_states=before_hidden_states.float(),
            control_factor=control_factor
        )
        hidden_states_temp = hidden_states_temp + hidden_states
        return (hidden_states_temp + self.mlp(self.post_attention_layernorm(hidden_states_temp)),) + (
...
class YuanModel(YuanPreTrainedModel):
    ...
    def _prepare_decoder_attention_mask_training(self, input_id, inputs_embeds, eod_token, reset_mask_flag,
    ...
    def forward(
        ...
        if self.training or self.reset_position_ids:
            attention_mask, _ = self._prepare_decoder_attention_mask_training(input_ids1, inputs_embeds, self.eod_token,
                                                                              reset_mask_flag,
...
class YuanForCausalLM(YuanPreTrainedModel):
    def __init__(self, config):
        ...
        self.save_hidden = [None] * self.num_layers
        # Initialize weights and apply final processing
        self.post_init()
    ...
    def forward(
        ...
        return torch.argmax(self.lm_head(last_hidden_states).half()).int()
