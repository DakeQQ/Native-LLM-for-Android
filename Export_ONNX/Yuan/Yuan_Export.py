...
max_seq_len = 1024  # Please modify the same variable, which declared in the modified yuan_hf_model.py on line 685, at the same time.
...
past_key_states = torch.zeros((num_layers, num_heads, max_seq_len, head_dim), dtype=torch.float16)
past_values_states = past_key_states
before_hidden_states = torch.zeros((num_layers, 1, 2, hidden_size), dtype=torch.float16)
...
past_key_states_A = np.zeros((num_layers, num_heads, max_seq_len, head_dim), dtype=np.float16)
past_values_states_A = past_key_states_A
before_hidden_states = np.zeros((num_layers, 1, 2, hidden_size), dtype=np.float16)
...
while history_len < max_single_chat_length:
    last_hidden_states, past_key_states_A, past_values_states_A, before_hidden_states = ort_session_A.run(
        ...
        {in_name_A0: input_ids,
         ...
         in_name_A5: ids_len,
         in_name_A6: before_hidden_states,
         in_name_A7: control_factor})
