...
class YuanRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        return (self.weight.half() * hidden_states * torch.rsqrt(hidden_states.pow(2).mean(-1, keepdim=True) + self.variance_epsilon)).float()
...
class YuanAttention(nn.Module):
    ...
    def __init__(self, config: YuanConfig):
        ...
        self.softmax_scale = torch.tensor([1.0 / math.sqrt(self.head_dim)], dtype=torch.float16)
    ...
    def forward(
        ...
        (query_states, key_states) = torch.chunk(torch.cat([self.q_proj(hidden_states).half(), self.k_proj(hidden_states).half()], dim=-1).view(ids_len, self.num_heads, self.num_heads_2), 2, dim=-1)
