...
class INTERNLM(torch.nn.Module):
    def __init__(self, internlm, max_seq_len, num_heads, num_key_value_heads, head_dim, num_layers):
        ...
        base = self.internlm.config.rope_theta * ((self.internlm.config.rope_scaling['factor'] * (max_seq_len + 1) / float(self.internlm.config.max_position_embeddings)) - (self.internlm.config.rope_scaling['factor'] - 1)) ** float(head_dim / (head_dim - 2))
