...
path = '/home/DakeQQ/Downloads/internlm2_5-1_8b-chat'                     # Set the folder path where the InternLM whole project downloaded.
onnx_model_A = '/home/DakeQQ/Downloads/InternLM_ONNX/InternLM.onnx'       # Assign a path where the exported InternLM model stored.

# Load the model
shutil.copyfile("./modeling_modified/modeling_internlm2.py", path + "/modeling_internlm2.py")
...
max_seq_len = 1024                                                      # Please modify the same variable, which declared in the modified modeling_internlm2.py on line 916, at the same time.
...
model.model.tok_embeddings.weight.requires_grad = False
data = model.model.tok_embeddings.weight.data
...
session_opts.log_severity_level = 3         # 3 = error level, it an adjustable value.
session_opts.log_verbosity_level = 3        # 3 = error level, it an adjustable value.
...
while (history_len < max_single_chat_length) or (num_decode < max_single_chat_length):
    ...
    if token_id in [1, 92542]:  # the stop_id in InternLM
        break
    else:
        history_len += ids_len
        if num_decode < 1:
