...
download_path                  = r'D:\LLM\Seed-X-PPO-7B'                         # Set the folder path where the Seed-X-[PRO-7B, Instruct-7B] whole project downloaded.
onnx_model_Embed               = r'D:\LLM\Seed_X_ONNX\LLM_Embed.onnx'
onnx_model_Main                = r'D:\LLM\Seed_X_ONNX\LLM_Main.onnx'
onnx_model_Rotary_Text_Prefill = r'D:\LLM\Seed_X_ONNX\Rotary_Text_Prefill.onnx'
onnx_model_Rotary_Text_Decode  = r'D:\LLM\Seed_X_ONNX\Rotary_Text_Decode.onnx'
onnx_model_Greedy              = r'D:\LLM\Seed_X_ONNX\Greedy_Search.onnx'
onnx_model_First_Beam          = r'D:\LLM\Seed_X_ONNX\First_Beam_Search.onnx'
onnx_model_Second_Beam         = r'D:\LLM\Seed_X_ONNX\Second_Beam_Search.onnx'
onnx_model_Penalty             = r'D:\LLM\Seed_X_ONNX\Apply_Penalty.onnx'
onnx_model_Argmax              = r'D:\LLM\Seed_X_ONNX\Argmax.onnx'
onnx_model_KV_Slice            = r'D:\LLM\Seed_X_ONNX\KV_Slice.onnx'


# Test input
TEST_SENTENCE            = "May the force be with you"
ORIGINAL_LANGUAGE        = "English"
TARGET_LANGUAGE          = "Chinese"
LOCAL_TOKENIZER_PATH     = os.path.dirname(__file__) or "."
...
STOP_TOKEN               = [2]                     # Seed-X stop token ids
...
def get_language(language_name):
    language_data = {}
    _lang_entries = [
        # English Key          Chinese Key        Abbr   English Name
        ("arabic",             "阿拉伯语",        "ar",  "Arabic"),
        ("chinese",            "中文",            "zh",  "Chinese"),
        ("croatian",           "捷克语",          "cs",  "Czech"),
        ("danish",             "丹麦语",          "da",  "Danish"),
        ("dutch",              "荷兰语",          "nl",  "Dutch"),
        ("english",            "英语",            "en",  "English"),
        ("finnish",            "芬兰语",          "fi",  "Finnish"),
        ("french",             "法语",            "fr",  "French"),
        ("german",             "德语",            "de",  "German"),
        ("hungarian",          "匈牙利语",        "hu",  "Hungarian"),
        ("indonesian",         "印度尼西亚语",    "id",  "Indonesian"),
        ("italian",            "意大利语",        "it",  "Italian"),
        ("japanese",           "日语",            "ja",  "Japanese"),
        ("korean",             "韩语",            "ko",  "Korean"),
        ("malay",              "马来语",          "ms",  "Malay"),
        ("norwegian",          "挪威语",          "no",  "Norwegian"),
        ("norwegian bokmal",   "挪威博克马尔语",  "nb",  "Norwegian Bokmal"),
        ("polish",             "波兰语",          "pl",  "Polish"),
        ("portuguese",         "葡萄牙语",        "pt",  "Portuguese"),
        ("romanian",           "罗马尼亚语",      "ro",  "Romanian"),
        ("russian",            "俄语",            "ru",  "Russian"),
        ("spanish",            "西班牙语",        "es",  "Spanish"),
        ("swedish",            "瑞典语",          "sv",  "Swedish"),
        ("thai",               "泰语",            "th",  "Thai"),
        ("turkish",            "土耳其语",        "tr",  "Turkish"),
        ("ukrainian",          "乌克兰语",        "uk",  "Ukrainian"),
        ("vietnamese",         "越南语",          "vi",  "Vietnamese"),
    ]
    for en_key, zh_key, abbr, english_name in _lang_entries:
        entry = {"abbr": abbr, "english_name": english_name}
        language_data[en_key] = entry
        language_data[zh_key] = entry

    normalized = language_name.lower()
    if normalized not in language_data:
        return None, None

    metadata = language_data[normalized]
    return f"<{metadata['abbr']}>", metadata["english_name"]
...
class ROTARY_MASK_PREFILL(torch.nn.Module):
    ...
    def _build_rotary_table(llm, max_seq_len):
        ...
        rotary_emb   = getattr(llm.model, "rotary_emb", None)
...
class LLM_MAIN(torch.nn.Module):
    ...
    def _fuse_qkv_projection(self, layer, scale_factor, norm_factor):
        """Fuse Q, K, V projections and absorb input RMSNorm into the merged weight."""
        ...
        qkv.weight.copy_(torch.cat([
            q_proj.weight * scale_factor,
            k_proj.weight * scale_factor,
            v_proj.weight,
        ], dim=0))
    ...
    def forward(self, *all_inputs):
        ...
        for i, layer in enumerate(self.llm.model.layers):
            ...
            # Apply RoPE once to the fused Q+K slice.
...
if DO_EXPORT:
    ...
    with (torch.inference_mode()):
        ...
        head_dim     = getattr(model.config, 'head_dim', None) or model.model.layers[0].self_attn.head_dim
    ...
    print(
        '\nExport done!\n\n'
        'Start running Seed-X by ONNXRuntime.\n'
...
_, source_language = get_language(ORIGINAL_LANGUAGE)
target_abbr, target_language = get_language(TARGET_LANGUAGE)
if source_language is None or target_language is None or target_abbr is None:
    raise ValueError("Unsupported translation language configuration.")

translation_request = f"Translate the following {source_language} sentence into {target_language}:"
prompt = f"{translation_request}\n{TEST_SENTENCE} {target_abbr}"
...
# DECODE LOOP
# ══════════════════════════════════════════════════════════════════════════════
print(f'\nTest Query: {translation_request}\n\n{TEST_SENTENCE}\n\nSeed-X Answering:')
...
while num_decode < generate_limit:
    ...
    if USE_BEAM_SEARCH:
        ...
        # Streaming print
        print(tokenizer.decode([max_logits_idx], skip_special_tokens=True), end="", flush=True)
