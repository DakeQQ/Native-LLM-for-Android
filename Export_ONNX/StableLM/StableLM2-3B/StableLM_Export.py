...
model_folder_path = 'C:/Users/Downloads/StableLM-Zephyr-3B '  # set the folder path where the StableLM whole project downloaded.
modified_path_A = 'C:./modeling_modified/modeling_stablelm.py'  # The path where store the modified part_A modeling_stablelm.py
modified_path_B = 'C:./modeling_modified/modeling_stablelm.py'  # The path where store the modified part_B modeling_stablelm.py
...
onnx_model_A = 'C:/Users/Downloads/StableLM_ONNX/StableLM_A.onnx'  # Assign a path where the exported part_A StableLM model stored.
onnx_model_B = 'C:/Users/Downloads/StableLM_ONNX/StableLM_B.onnx'  # Assign a path where the exported part_B StableLM model stored.
...
max_seq_len = 1024  # Please modify the same variable, which declared in the modified modeling_stablelm.py on line 860, at the same time.
...
shutil.copyfile(modified_path_B, transformers_stablelm_path)
model = AutoModelForCausalLM.from_pretrained(model_folder_path, torch_dtype=torch.float32, device_map='cpu', trust_remote_code=True).float().eval()
torch.onnx.export(
    model, (
...
ort_session_B = onnxruntime.InferenceSession(onnx_model_A, sess_options=session_opts,  providers=['CPUExecutionProvider'])
