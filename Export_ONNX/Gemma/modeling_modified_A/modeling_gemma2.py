...
class Gemma2SdpaAttention(Gemma2Attention):
    ...
    def forward(
        ...
        return self.o_proj(torch.matmul(nn.functional.softmax(torch.tanh(torch.matmul(query_states * rotary_pos_emb_cos + rotate_half(query_states, self.head_dim_half) * rotary_pos_emb_sin, key_states.transpose(1,2).float()) * self.scaling) * self.config.attn_logit_softcapping
            + attention_mask, dim=-1, dtype=torch.float32), value_states.float()).transpose(0, 1).contiguous().view(-1, self.hidden_size)), save_key_states, save_value_states
...
class Gemma2ForCausalLM(Gemma2PreTrainedModel):
    ...
    def forward(
        ...
        return torch.cat(((self.embed_data[ids] * self.scale[ids] + self.zero_point[ids]).half(), torch.zeros((self.max_seq_len - ids_len, self.hidden_size), dtype=torch.float16)), dim=-2)
