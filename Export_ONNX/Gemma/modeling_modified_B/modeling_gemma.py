...
class GemmaRMSNorm(nn.Module):
    def __init__(self, dim: int, eps: float = 1e-6):
        super().__init__()
        self.eps = torch.tensor([eps], dtype=torch.float16)
    ...
    def forward(self, x):
        ...
        return (self._norm(x) * (1.0 + self.weight.half())).float()
    ...
    def forward(
        ...
        return self.o_proj(torch.matmul(nn.functional.softmax(
            torch.matmul((query_states * rotary_pos_emb_cos) + (rotate_half(query_states) * rotary_pos_emb_sin),
                         key_states.transpose(1, 2)) * self.head_dim_factor + attention_mask, dim=-1, dtype=torch.float16),
...
class GemmaForCausalLM(GemmaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.scale_emb = torch.tensor(self.config.hidden_size**0.5, dtype=torch.float32)
