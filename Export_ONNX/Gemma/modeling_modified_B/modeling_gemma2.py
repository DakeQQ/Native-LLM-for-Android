    ...
    def forward(
        ...
        return self.o_proj(torch.matmul(nn.functional.softmax(
            torch.tanh(torch.matmul(query_states * rotary_pos_emb_cos + rotate_half(query_states) * rotary_pos_emb_sin,
                                    key_states.transpose(1, 2)) * self.scaling) * self.config.attn_logit_softcapping
            + attention_mask, dim=-1, dtype=torch.float32), value_states).transpose(0, 1).contiguous().view(ids_len, -1)), save_key_states, save_value_states
