...
class Gemma2RMSNorm(nn.Module):
    ...
    def _norm(self, x):
        return x / torch.sqrt((x * x).mean(dim=-1, keepdim=True) + self.eps)
...
class Gemma2Attention(nn.Module):
    ...
    class Gemma2Attention(nn.Module):
        ...
        def __init__(self, config: Gemma2Config, layer_idx: Optional[int] = None):
            ...
            self.rope_theta = config.rope_theta
            self.is_causal = True
            self.scaling = (config.query_pre_attn_scalar ** -0.5) / config.attn_logit_softcapping
        ...
        def forward(
            ...
            return self.o_proj(torch.matmul(nn.functional.softmax(torch.tanh(torch.matmul(
                query_states * rotary_pos_emb_cos + rotate_half(query_states, self.head_dim_half) * rotary_pos_emb_sin,
                key_states.transpose(1, 2)) * self.scaling) * self.config.attn_logit_softcapping + attention_mask, dim=-1, dtype=torch.float32), value_states).transpose(0, 1).contiguous().view(-1, self.hidden_size)), save_key_states, save_value_states
...
class Gemma2SdpaAttention(Gemma2Attention):
    ...
    def forward(
        ...
        key_states = repeat_kv(key_states, self.num_key_value_groups, self.num_key_value_heads_mul_groups, self.head_dim, kv_seq_len)
        value_states = repeat_kv(value_states, self.num_key_value_groups, self.num_key_value_heads_mul_groups, self.head_dim, kv_seq_len)
        return self.o_proj(torch.matmul(nn.functional.softmax(torch.tanh(torch.matmul(query_states * rotary_pos_emb_cos + rotate_half(query_states, self.head_dim_half) * rotary_pos_emb_sin,
                                    key_states.transpose(1, 2)) * self.scaling) * self.config.attn_logit_softcapping + attention_mask, dim=-1, dtype=torch.float32), value_states).transpose(0, 1).contiguous().view(-1, self.hidden_size)), save_key_states, save_value_states
...
class Gemma2DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        return hidden_states_temp + self.post_feedforward_layernorm(self.mlp(self.pre_feedforward_layernorm(hidden_states_temp))), past_key_states, past_value_states
