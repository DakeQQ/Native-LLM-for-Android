...
modified_path = 'modeling_modified/modeling_llama.py'  # The path where the modified modeling_llama.py stored.
onnx_model_A = '/home/dake/Downloads/Llama_ONNX/llama.onnx'  # Assign a path where the exported llama model stored.
transformers_llama_path = '/home/dake/anaconda3/envs/python_311b/lib/python3.11/site-packages/transformers/models/llama/modeling_llama.py'  # The original modeling_llama.py path which was stored in the transformers python package.

# Load the model
shutil.copyfile(modified_path, transformers_llama_path)
model = AutoModelForCausalLM.from_pretrained(path, torch_dtype=torch.float32, device_map='cpu', trust_remote_code=True).eval()
max_seq_len = 1024  # Please modify the same variable, which declared in the modified modeling_llama.py on line 993, at the same time.
...
for i in range(num_layers):
    ...
    layer_attn.qkv_proj = torch.nn.Linear(qkv_weight.shape[1], qkv_weight.shape[0], bias=False)
