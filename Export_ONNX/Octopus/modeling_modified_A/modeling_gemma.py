...
class GemmaSdpaAttention(GemmaAttention):
    ...
    def forward(
        ...
        return self.o_proj(torch.matmul(nn.functional.softmax(
            ...
            dtype=torch.float32), value_states).transpose(0, 1).reshape(ids_len, self.hidden_size)).contiguous(), save_key_states.half(), save_value_states.half()
...
class GemmaForCausalLM(GemmaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.scale_emb = torch.tensor(self.hidden_size**0.5, dtype=torch.float32)
    ...
    def forward(
        ...
        return (torch.cat((self.model.embed_tokens(input_ids[:ids_len]) * self.scale_emb, torch.zeros((self.max_seq_len - ids_len, self.hidden_size), dtype=torch.float32)), dim=-2),)
