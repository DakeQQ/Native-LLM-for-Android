...
class InternLM2Attention(nn.Module):
    ...
    def forward(
        ...
        key_states = qkv_states[..., -2, :].permute(1, 2, 0)
        ...
        return self.wo(torch.matmul(nn.functional.softmax(torch.matmul(query_states * rotary_pos_emb_cos_q + rotate_half(query_states, self.head_dim_half, -1) * rotary_pos_emb_sin_q, key_states) * self.head_dim_factor
...
class InternLM2VEForCausalLM(InternLM2PreTrainedModel):
    ...
    def forward(
        ...
        hidden_states = all_inputs[-1].float()
        split_factor = all_inputs[-2]
        attention_mask = all_inputs[-3]
        ...
        return (torch.argmax(self.output(self.model.norm(hidden_states[:, -1])), dim=-1, keepdim=True).int(),
                *self.save_key,
                *self.save_value)
