...
def rms_layernorm(hidden: torch.Tensor, weight: torch.Tensor, eps: torch.Tensor):
    return (hidden * weight * torch.rsqrt(hidden.pow(2).mean(dim=-1, keepdim=True) + eps)).float()
...
class MiniCPMRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        return rms_layernorm(hidden_states, self.weight.half(), self.variance_epsilon)
...
class MiniCPMSdpaAttention(MiniCPMAttention):
    ...
    def forward(
        ...
        query_states = self.q_proj(hidden_states).half().view(ids_len, self.num_heads, self.head_dim).transpose(0, 1)
        key_states = self.k_proj(hidden_states).half().view(ids_len, self.num_key_value_heads, self.head_dim).transpose(0, 1)
        ...
        return self.o_proj(torch.matmul(nn.functional.softmax(
            ...
            dtype=torch.float16), value_states).transpose(0, 1).reshape(ids_len, self.hidden_size).float().contiguous()), save_key_states, save_value_states
...
class MiniCPMForCausalLM(MiniCPMPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.num_key_value_heads = config.num_key_value_heads
        self.head_dim = self.hidden_size // self.num_heads
        self.factor = self.hidden_size / config.dim_model_base
    ...
    def forward(
        ...
        hidden_states = self.model.embed_tokens(input_ids[:ids_len]) * torch.tensor([self.config.scale_emb], dtype=torch.float16)
        ...
        return (torch.argmax(self.lm_head(self.model.norm(hidden_states[-1])).half()).int(),
