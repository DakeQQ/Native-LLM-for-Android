...
class MiniCPMSdpaAttention(MiniCPMAttention):
    ...
    def forward(
        ...
        return self.o_proj(torch.matmul(nn.functional.softmax(torch.matmul(query_states * rotary_pos_emb_cos + rotate_half(query_states, self.head_dim_half) * rotary_pos_emb_sin,
                         key_states.transpose(1, 2)) * self.head_dim_factor + attention_mask, dim=-1, dtype=torch.float32), value_states).transpose(0, 1).contiguous().view(-1, self.hidden_size)), save_key_states, save_value_states
...
class MiniCPMDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        return (hidden_states_temp + self.mlp(self.post_attention_layernorm(hidden_states_temp)) * self.scale_factor), past_key_states, past_value_states
...
class MiniCPMForCausalLM(MiniCPMPreTrainedModel):
    ...
    def forward(
        ...
        cos_rotary_pos_emb = self.cos_rotary_pos_emb[:, history_len:kv_seq_len, :].flaot()
        sin_rotary_pos_emb = self.sin_rotary_pos_emb[:, history_len:kv_seq_len, :].flaot()
