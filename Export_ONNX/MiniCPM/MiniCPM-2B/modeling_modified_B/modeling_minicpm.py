...
def rms_layernorm(hidden: torch.Tensor, weight: torch.Tensor, eps: float):
    return hidden * weight * torch.rsqrt(hidden.pow(2).mean(dim=-1, keepdim=True) + eps)
...
class MiniCPMSdpaAttention(MiniCPMAttention):
    ...
    def forward(
        ...
        return self.o_proj(torch.matmul(nn.functional.softmax(
            ...
            dtype=torch.float32), value_states.float()).transpose(0, 1).reshape(1, ids_len, self.hidden_size).contiguous()), key_states, value_states
...
class MiniCPMForCausalLM(MiniCPMPreTrainedModel):
    ...
    def forward(
        ...
        cos_rotary_pos_emb = cos_rotary_pos_emb[:, history_len:kv_seq_len, :].float()
        sin_rotary_pos_emb = sin_rotary_pos_emb[:, history_len:kv_seq_len, :].float()
