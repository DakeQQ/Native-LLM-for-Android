...
class MiniCPMAttention(nn.Module):
    ...
    def __init__(self, config: MiniCPMConfig, layer_idx: Optional[int] = None):
        ...
        self.is_causal = True
        self.head_dim_factor = torch.tensor([1.0 / math.sqrt(self.head_dim)], dtype=torch.float16)
    ...
    def forward(
        ...
        return self.o_proj(torch.matmul(nn.functional.softmax(
            torch.matmul(query_states * rotary_pos_emb_cos + rotate_half(query_states) * rotary_pos_emb_sin,
                         key_states.transpose(1, 2).float()) * self.head_dim_factor + attention_mask, dim=-1,
            dtype=torch.float16), value_states.float()).transpose(0, 1).reshape(ids_len, self.hidden_size).contiguous()), key_states, value_states
...
class MiniCPMDecoderLayer(nn.Module):
    def __init__(self, config: MiniCPMConfig, layer_idx: int):
        ...
        self.scale_factor = torch.tensor([self.scale_depth / math.sqrt(self.num_hidden_layers)], dtype=torch.float32)
...
class MiniCPMForCausalLM(MiniCPMPreTrainedModel):
    ...
    def forward(
        ...
        expand_space_B = self.expand_space_B[:, :, :(self.max_seq_len - kv_seq_len), :]
        return (torch.cat((hidden_states.half(), expand_space_A[ids_len:, :]), dim=-2),
