...
download_path                  = r'/home/DakeQQ/Downloads/MiniCPM5-1B'                             # Set the folder path where the MiniCPM5 project was downloaded.
...
THINKING                 = False                   # True: enable thinking (model generates <think>...</think> before answering); False: disable thinking (force skip with empty <think></think> tokens)
PREVENT_F16_OVERFLOW     = False                   # Prevent float16 overflow. Set True for Q4F16 or Q8F16 or F16 quantization.
STOP_TOKEN               = [1, 130073]             # MiniCPM5 stop token ids
...
class LLM_EMBED(torch.nn.Module):
    ...
    def forward(self, input_ids):
        return self.embed_tokens(input_ids.to(torch.long))
...
class ROTARY_MASK_PREFILL(torch.nn.Module):
    def __init__(self, llm, max_seq_len):
        ...
        inv_freq = llm.model.rotary_emb.inv_freq.float()
        position_ids = torch.arange(max_seq_len, dtype=torch.float32).unsqueeze(-1)
        freqs = position_ids * inv_freq
        emb = torch.cat([freqs, freqs], dim=-1)
        self.register_buffer('cos_rotary_pos_emb', emb.cos().unsqueeze(0).unsqueeze(2).unsqueeze(2).half(), persistent=False)
        self.register_buffer('sin_rotary_pos_emb', emb.sin().unsqueeze(0).unsqueeze(2).unsqueeze(2).half(), persistent=False)
...
class LLM_MAIN(torch.nn.Module):
    def __init__(self, llm, num_heads, num_key_value_heads, head_dim, num_layers):
        ...
        self.total_qkv_heads = num_heads + num_key_value_heads + num_key_value_heads
        self.qk_v_split_sizes = [num_heads + num_key_value_heads, num_key_value_heads]
    ...
    def _fuse_weights(self):
        ...
        norm_factor = self.hidden_size_sqrt
    ...
    def forward(self, *all_inputs):
        ...
        for i, layer in enumerate(self.llm.model.layers):
            ...
            qk, v = torch.split(qkv, self.qk_v_split_sizes, dim=-2)
            ...
            current_k = k.permute(0, 3, 2, 4, 1)
            current_v = v.transpose(1, 3)
            past_k = all_inputs[i]
            past_v = all_inputs[i + self.num_layers]

            if self.kv_rotary_q4:
                if self.kv_sym:
                    packed_k, scale_k, packed_v, scale_v = self.quantizer(current_k, current_v, batch_size, self.num_key_value_heads, self.kv_pack_quarter)
                    k = torch.cat([past_k, packed_k], dim=-1)
                    v = torch.cat([past_v, packed_v], dim=-2)
                    ...
                    packed_k, scale_k, bias_k, packed_v, scale_v, bias_v = self.quantizer(current_k, current_v, batch_size, self.num_key_value_heads, self.kv_pack_quarter)
                    k = torch.cat([past_k, packed_k], dim=-1)
                    v = torch.cat([past_v, packed_v], dim=-2)
                    k_s = torch.cat([all_inputs[i + self.num_layers_2], scale_k], dim=-1)
                ...
                if self.kv_sym:
                    ...
                    past_k_s = all_inputs[i + self.num_layers_2]
                    if USE_FLOAT16_SCALE_BIAS:
                        past_k_s = past_k_s.float()
                        scale_k = scale_k.float()
                        v_s = v_s.float()
                    ...
                    if self.kv_q8_grouped:
                        ...
                        if self.kv_rotary_q8_cuda:
                            k_hist = self.quantizer.unpack_cuda(k_hist, -2, batch_size, self.num_key_value_heads, self.kv_unpack_head_dim)
                        k_hist = self.quantizer._decode_signed_q8_storage(k_hist).float()
                        k_hist = k_hist.view(batch_size, self.num_key_value_heads, 1, self.quantizer.kv_quant_num_groups, self.quantizer.kv_quant_group_size, -1)
                        attn_hist = (torch.matmul(q_rot_g, k_hist) * past_k_s).sum(dim=-3)

                        k_curr = packed_k
                        if self.kv_rotary_q8_cuda:
                            k_curr = self.quantizer.unpack_cuda(k_curr, -2, batch_size, self.num_key_value_heads, self.kv_unpack_head_dim)
                        k_curr = self.quantizer._decode_signed_q8_storage(k_curr).float()
                        k_curr = k_curr.view(batch_size, self.num_key_value_heads, 1, self.quantizer.kv_quant_num_groups, self.quantizer.kv_quant_group_size, -1)
                        attn_curr = (torch.matmul(q_rot_g, k_curr) * scale_k).sum(dim=-3) + attention_mask
                        attn = torch.softmax(torch.cat([attn_hist, attn_curr], dim=-1), dim=-1, dtype=torch.float32)

                        v_signed = v
                        if self.kv_rotary_q8_cuda:
                            v_signed = self.quantizer.unpack_cuda(v_signed, -1, batch_size, self.num_key_value_heads, self.kv_unpack_head_dim)
                        v_signed = self.quantizer._decode_signed_q8_storage(v_signed).float()
                        ...
                        if self.kv_rotary_q8_cuda:
                            k_hist = self.quantizer.unpack_cuda(k_hist, -2, batch_size, self.num_key_value_heads, self.kv_unpack_head_dim)
                        k_hist = self.quantizer._decode_signed_q8_storage(k_hist).float()
                        attn_hist = torch.matmul(q_rot, k_hist) * past_k_s

                        k_curr = packed_k
                        if self.kv_rotary_q8_cuda:
                            k_curr = self.quantizer.unpack_cuda(k_curr, -2, batch_size, self.num_key_value_heads, self.kv_unpack_head_dim)
                        k_curr = self.quantizer._decode_signed_q8_storage(k_curr).float()
                        attn_curr = torch.matmul(q_rot, k_curr) * scale_k + attention_mask
                        attn = torch.softmax(torch.cat([attn_hist, attn_curr], dim=-1), dim=-1, dtype=torch.float32)
                    ...
                    past_k_s = all_inputs[i + self.num_layers_2]
                    past_k_b = all_inputs[i + self.num_layers_3]
                    if USE_FLOAT16_SCALE_BIAS:
                        past_k_s = past_k_s.float()
                        past_k_b = past_k_b.float()
                        scale_k = scale_k.float()
                        bias_k = bias_k.float()
                        v_s = v_s.float()
                        v_b = v_b.float()
                    ...
                    if self.kv_q8_grouped:
                        ...
                        if self.kv_rotary_q8_cuda:
                            k_hist = self.quantizer.unpack_cuda(k_hist, -2, batch_size, self.num_key_value_heads, self.kv_unpack_head_dim)
                        k_hist = k_hist.float().view(batch_size, self.num_key_value_heads, 1, self.quantizer.kv_quant_num_groups, self.quantizer.kv_quant_group_size, -1)
                        q_sum_g = q_rot_g.sum(dim=-1, keepdim=True)
                        attn_hist = (torch.matmul(q_rot_g, k_hist) * past_k_s + q_sum_g * past_k_b).sum(dim=-3)

                        k_curr = packed_k
                        if self.kv_rotary_q8_cuda:
                            k_curr = self.quantizer.unpack_cuda(k_curr, -2, batch_size, self.num_key_value_heads, self.kv_unpack_head_dim)
                        k_curr = k_curr.float().view(batch_size, self.num_key_value_heads, 1, self.quantizer.kv_quant_num_groups, self.quantizer.kv_quant_group_size, -1)
                        attn_curr = (torch.matmul(q_rot_g, k_curr) * scale_k + q_sum_g * bias_k).sum(dim=-3) + attention_mask
                        attn = torch.softmax(torch.cat([attn_hist, attn_curr], dim=-1), dim=-1, dtype=torch.float32)

                        v_dequant = v
                        if self.kv_rotary_q8_cuda:
                            v_dequant = self.quantizer.unpack_cuda(v_dequant, -1, batch_size, self.num_key_value_heads, self.kv_unpack_head_dim)
                        v_q_g = v_dequant.float().view(batch_size, self.num_key_value_heads, 1, -1, self.quantizer.kv_quant_num_groups, self.quantizer.kv_quant_group_size)
                        v_dequant = (v_q_g * v_s + v_b).reshape(batch_size, self.num_key_value_heads, 1, -1, self.head_dim)
                        attn = torch.matmul(attn, v_dequant)
                        if self.quantizer.use_hadamard:
                            attn = self.quantizer.inverse_hadamard_attn(attn, batch_size)
                        ...
                        if self.kv_rotary_q8_cuda:
                            k_hist = self.quantizer.unpack_cuda(k_hist, -2, batch_size, self.num_key_value_heads, self.kv_unpack_head_dim)
                        k_hist = k_hist.float()
                        attn_hist = torch.addcmul(
                            q_bias_factor * past_k_b,
                            torch.matmul(q_rot, k_hist),
                            past_k_s,
                        )
                        ...
                        if self.kv_rotary_q8_cuda:
                            k_curr = self.quantizer.unpack_cuda(k_curr, -2, batch_size, self.num_key_value_heads, self.kv_unpack_head_dim)
                        k_curr = k_curr.float()
                        attn_curr = torch.addcmul(
                            q_bias_factor * bias_k,
                            torch.matmul(q_rot, k_curr),
                            scale_k,
                        ) + attention_mask
                        attn = torch.softmax(torch.cat([attn_hist, attn_curr], dim=-1), dim=-1, dtype=torch.float32)
                        ...
                        v_scaled = v
                        if self.kv_rotary_q8_cuda:
                            v_scaled = self.quantizer.unpack_cuda(v_scaled, -1, batch_size, self.num_key_value_heads, self.kv_unpack_head_dim)
                        v_scaled = v_scaled.float() * v_s
                ...
                if self.kv_sym:
                    packed_k, scale_k, packed_v, scale_v = self.quantizer(current_k, current_v, batch_size, self.num_key_value_heads, self.head_dim_quarter)
                    k = torch.cat([past_k, packed_k], dim=-1)
                    v = torch.cat([past_v, packed_v], dim=-2)
                    k_s = torch.cat([all_inputs[i + self.num_layers_2], scale_k], dim=-1)
                    if self.kv_q8_grouped:
                    ...
                    if self.kv_q8_grouped:
                        q_g = q
                        if self.quantizer.use_shuffle:
                            q_g = q_g.index_select(-1, self.quantizer.shuffle_idx)
                        q_g = q_g.view(batch_size, self.num_key_value_heads, self.num_key_value_groups, -1, self.quantizer.kv_quant_num_groups, self.quantizer.kv_quant_group_size)
                    ...
                    packed_k, scale_k, bias_k, packed_v, scale_v, bias_v = self.quantizer(current_k, current_v, batch_size, self.num_key_value_heads, self.head_dim_quarter)
                    k = torch.cat([past_k, packed_k], dim=-1)
                    v = torch.cat([past_v, packed_v], dim=-2)
                    k_s = torch.cat([all_inputs[i + self.num_layers_2], scale_k], dim=-1)
                    k_b = torch.cat([all_inputs[i + self.num_layers_3], bias_k], dim=-1)
                ...
                if self.kv_f16:
                    current_k = current_k.half()
                    current_v = current_v.half()
                ...
                if self.kv_f16:
                    past_k = past_k.float()
                    past_v = past_v.float()
                    current_k = current_k.float()
                    current_v = current_v.float()

                attn_hist = torch.matmul(q, past_k)
                attn_curr = torch.matmul(q, current_k) + attention_mask
                attn = torch.softmax(torch.cat([attn_hist, attn_curr], dim=-1), dim=-1, dtype=torch.float32)
                attn = torch.matmul(attn, torch.cat([past_v, current_v], dim=-2))
...
if DO_EXPORT:
    ...
    with torch.inference_mode():
        ...
        vocab_size = int(getattr(model.config, 'vocab_size', model.vocab_size))
        ...
        _q8_grouped = (_is_quantized or KV_QUANT_DTYPE in ('ROTARY_Q8', 'ROTARY_Q8_CUDA')) and (USE_HADAMARD or USE_SHUFFLE) and KV_QUANT_GROUP_SIZE < head_dim
        _grouped_6d = _is_rotary_q4 or _q8_grouped
...
# --- Step 2: Compute rotary embeddings & local mask (prefill) ---
