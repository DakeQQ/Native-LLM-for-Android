...
path = '/home/iamj/Downloads/MiniCPM4-0.5B'                              # Set the folder path where the MiniCPM4 whole project downloaded.
onnx_model_A = '/home/iamj/Downloads/MiniCPM_Optimized/MiniCPM.onnx'     # Assign a path where the exported Qwen model stored.
...
class MINICPM(torch.nn.Module):
    def __init__(self, minicpm, max_seq_len, num_heads, num_key_value_heads, head_dim, num_layers):
        ...
        self.cos_rotary_pos_emb = self.minicpm.model.layers._modules['0'].self_attn.rotary_emb.cos_cached[:max_seq_len].unsqueeze(0).half()
        self.sin_rotary_pos_emb = self.minicpm.model.layers._modules['0'].self_attn.rotary_emb.sin_cached[:max_seq_len].unsqueeze(0).half()
    ...
    def forward(self, *all_inputs):
        ...
        return *self.save_key, *self.save_value, torch.argmax(self.minicpm.lm_head(hidden_states), dim=-1, keepdim=True).int(), kv_seq_len
...
with torch.inference_mode():
    ...
    model = MINICPM(model, MAX_SEQ_LEN, num_heads, num_key_value_heads, head_dim, num_layers)
