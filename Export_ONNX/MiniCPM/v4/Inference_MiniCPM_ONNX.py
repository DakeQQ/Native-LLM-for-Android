...
def parse_args():
    parser = argparse.ArgumentParser(description="Run the MiniCPM4 merged-ONNX inference demo.")
    parser.add_argument(
        "--model-folder",
        type=Path,
        default=Path(__file__).resolve().parent / "MiniCPM_Optimized",
        help="Folder containing merged ONNX graphs and LLM_SharedInitializers.onnx(.data).",
    )
    parser.add_argument(
        "--tokenizer-folder",
        type=Path,
        default=Path.home() / "Downloads" / "MiniCPM4-0.5B",
...
def load_metadata_carrier(model_path):
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Re-export with MiniCPM_Export.py or copy the metadata carrier into the model folder."
        ) from exc

    metadata = meta_session.get_modelmeta().custom_metadata_map
    if not metadata.get('native_llm_metadata_version'):
        raise RuntimeError(
            "LLM_Metadata.onnx carries no Native-LLM metadata_props. Re-export the model with MiniCPM_Export.py."
        )
    return metadata
...
def load_model_file_names(meta):
    ...
    if missing:
        raise RuntimeError(
            ...
            "Re-export with MiniCPM_Export.py or re-quantize with Optimize_ONNX.py. Missing: "
            + ", ".join(missing)
        )
...
print(
    '\nStart running the MiniCPM4 LLM by ONNXRuntime.\n'
...
def run_merged_iobinding(folder, meta, strategy, model_file_names):
    ...
    if bos_token_id is not None and (tokens.shape[-1] == 0 or int(tokens[0, 0]) != bos_token_id):
        tokens = np.concatenate([np.full((tokens.shape[0], 1), bos_token_id, dtype=tokens.dtype), tokens], axis=1)
...
if _missing_shared_files:
    raise RuntimeError(
        ...
        f"{onnx_folder}. This runtime is merged-only; re-export with MiniCPM_Export.py or "
