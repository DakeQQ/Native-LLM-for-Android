"""Build MiniCPM4 merged ONNX graphs backed by one shared initializer bundle.
...
def transplant_quantized_main(target: onnx.ModelProto, quantized_primary: onnx.ModelProto) -> onnx.ModelProto:
    ...
    produced = {name for node in new_nodes for name in node.output if name}

    new_initializers: list[TensorProto] = []
    seen_inits: set[str] = set()

    def add_init(init: TensorProto) -> None:
        if init.name in seen_inits or init.name in produced:
            return
...
def build_shared_merged_bundle(
    ...
    (Main / RotaryPrefill / RotaryDecode / decode heads) and
