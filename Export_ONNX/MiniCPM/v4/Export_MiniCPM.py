"""Export MiniCPM4 ONNX graphs for optimized on-device inference."""
...
# Settings
MODEL_PATH                     = str(Path.home() / "Downloads" / "MiniCPM4-0.5B")  # HF checkpoint folder.
...
KV_QUANT_GROUP_SIZE            = 64                                  # MiniCPM4 head_dim; auto-clamped to a divisor.
...
class ROTARY_MASK_PREFILL(torch.nn.Module):
    ...
    def __init__(self, llm, max_seq_len):
        ...
        self.register_buffer("cos_rotary_pos_emb", torch.cat([cos, cos], dim=-1), persistent=False)
        self.register_buffer("sin_rotary_pos_emb", torch.cat([-sin, sin], dim=-1), persistent=False)

    @staticmethod
    def _build_rotary_table(llm, max_seq_len):
        rotary_emb = llm.model.layers[0].self_attn.rotary_emb
        original_max = getattr(rotary_emb, "original_max_position_embeddings", None)
        cached_len = int(getattr(rotary_emb, "max_seq_len_cached", 0))
        same_factor_regime = original_max is None or (
            (max_seq_len > original_max) == (cached_len > original_max)
        )
        if (
            cached_len >= max_seq_len
            and same_factor_regime
            and hasattr(rotary_emb, "cos_cached")
            and hasattr(rotary_emb, "sin_cached")
        ):
            head_dim_half = rotary_emb.inv_freq.numel()
            cos = rotary_emb.cos_cached[:max_seq_len, :head_dim_half].float()
            sin = rotary_emb.sin_cached[:max_seq_len, :head_dim_half].float()
        else:
            inv_freq = rotary_emb.inv_freq.detach().float()
            position_ids = torch.arange(max_seq_len, dtype=inv_freq.dtype)
            if original_max is None:
                freqs = torch.outer(position_ids, inv_freq)
            else:
                factors = rotary_emb.long_factor if max_seq_len > original_max else rotary_emb.short_factor
                factors = torch.as_tensor(factors, dtype=torch.float32)
                freqs = torch.outer(position_ids, 1.0 / factors) * inv_freq
            scaling_factor = float(getattr(rotary_emb, "scaling_factor", 1.0))
...
class LLM_MAIN(torch.nn.Module):
    ...
    def __init__(self, llm, num_heads, num_key_value_heads, head_dim, num_layers, hidden_size):
        ...
        # Keep the tied base matrix unchanged: both model-specific scales remain activations.
        final_output_scale = float(self.llm.config.dim_model_base / hidden_size)
        final_norm_weight = self.llm.model.norm.weight.detach().clone().float() * final_output_scale
        self.scale_emb = float(self.llm.config.scale_emb)
    ...
    def _fuse_weights(self, hidden_size):
        ...
        with torch.no_grad():
            for layer in self.llm.model.layers:
                ...
                residual_scale = float(layer.scale_depth / math.sqrt(layer.num_hidden_layers))
                self._scale_linear(layer.self_attn.o_proj, residual_scale)
                self._scale_linear(layer.mlp.down_proj, residual_scale)

            del self.llm.model.norm

    @staticmethod
    def _scale_linear(linear, scale):
...
def minicpm4_effective_rope(llm, seq_len):
    """Return MiniCPMLongRoPE's effective phase frequency and amplitude."""
    rotary_emb = llm.model.layers[0].self_attn.rotary_emb
    inv_freq = rotary_emb.inv_freq.detach().float()
    original_max = getattr(rotary_emb, "original_max_position_embeddings", None)
    if original_max is None:
        return inv_freq, float(getattr(rotary_emb, "scaling_factor", 1.0))

    factors = rotary_emb.long_factor if seq_len > original_max else rotary_emb.short_factor
    effective_inv_freq = inv_freq / torch.as_tensor(factors, dtype=torch.float32)
    return effective_inv_freq, float(rotary_emb.scaling_factor)
...
if __name__ == "__main__" and DO_EXPORT:
    ...
    with (torch.inference_mode()):
        ...
        model = AutoModelForCausalLM.from_pretrained(MODEL_PATH, torch_dtype=torch.float32, device_map='cpu', trust_remote_code=True, low_cpu_mem_usage=True).eval()
        ...
        rope_inv_freq, _ = minicpm4_effective_rope(model, MAX_SEQ_LEN)
        rope_inv_freq = rope_inv_freq.clone()
        ...
        onnx_metadata = build_model_metadata(
            ...
            MODEL_FILE_NAME_METADATA,
            {
                ...
                "scale_emb":                   float(_cfg.scale_emb),
                "scale_depth":                 float(_cfg.scale_depth),
                "dim_model_base":              int(_cfg.dim_model_base),
            },
            {
