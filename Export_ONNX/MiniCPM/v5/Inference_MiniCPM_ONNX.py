...
def parse_args():
    parser = argparse.ArgumentParser(description="Run the MiniCPM v5 merged-ONNX inference demo.")
    parser.add_argument(
        "--model-folder",
        type=Path,
    ...
    parser.add_argument(
        "--tokenizer-folder",
        type=Path,
        default=Path.home() / "Downloads" / "MiniCPM5-1B",
...
print(
    '\nStart running the MiniCPM v5 LLM by ONNXRuntime.\n'
...
def run_merged_iobinding(folder, meta, strategy, model_file_names):
    ...
    prompt = tokenizer.apply_chat_template(
        ...
        enable_thinking=TEST_THINK_MODE,
    )
