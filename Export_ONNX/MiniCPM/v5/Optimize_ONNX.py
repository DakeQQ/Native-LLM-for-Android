...
WEIGHT_ONLY_ALGORITHM          = "DEFAULT"                           # "k_quant" | "DEFAULT" | "RTN" | "HQQ". k_quant/RTN are Q4-only; use DEFAULT/HQQ for Q2/Q8.
...
UNIFY_EMBED_LMHEAD             = False                               # Enable only for checkpoints whose embedding and LM head are tied.
...
def _share_q4_embed_lmhead(graph, gather, embed_init: str, lmhead, vocab: int, hidden: int,
    ...
    qlo, qhi, qlo1, qhi1, qcat, qint = (make("qlo"), make("qhi"), make("qlo1"),
                                        make("qhi1"), make("qcat"), make("qint"))
    zlo, zhi, zlo1, zhi1, zcat, zflat, zint = (make("zlo"), make("zhi"), make("zlo1"),
                                               make("zhi1"), make("zcat"), make("zflat"), make("zint"))
    qf, zf, zf1, gs1, sub, deq = (make("qf"), make("zf"), make("zf1"), make("gs1"),
                                  make("sub"), make("deq"))
...
def build_quantized_merged_bundle(resolved: dict[str, ResolvedPlan]) -> None:
    ...
    if UNIFY_EMBED_LMHEAD and source_metadata.get("tie_word_embeddings", "0").lower() not in ("1", "true"):
        raise ValueError(
            "UNIFY_EMBED_LMHEAD=True is unsafe because metadata declares untied embedding and LM-head weights."
        )
