"""Optimize MiniCPM v5 ONNX modules and rebuild the shared merged bundle."""
...
# User config
ORIGINAL_FOLDER_PATH           = str(_SCRIPT_DIR / "MiniCPM_ONNX")      # Folder holding the exported *.onnx modules.
QUANTED_FOLDER_PATH            = str(_SCRIPT_DIR / "MiniCPM_Optimized") # Destination folder for the results.
DOWNLOAD_PATH                  = str(Path.home() / "Downloads" / "MiniCPM5-1B")  # Model dir (attention fusion); "NONE" to skip.
...
AFFINE_V2_CHUNK_BLOCKS         = 8192
AFFINE_V2_WEIGHTED_TOLERANCE   = 0.15
# Q4 sweeps every zero point. Q8 uses a seed-centered window to keep full-model
# refinement tractable; this must remain >= 16 to preserve Q4 behavior.
...
UNIFY_EMBED_LMHEAD             = True                                # Share only after exact exported-weight equality is verified.
...
MODEL_PLANS: dict[str, Plan] = {
    ...
    # Quantize Main (including its token embedding) once, then transplant that
    # block into the other merged shells and share its Q4 initializers on disk.
...
def _validate_quantized_embedding(primary_path: Path, primary_plan: ResolvedPlan) -> None:
    if primary_plan.method not in _WEIGHT_ONLY_BITS or "Gather" not in primary_plan.op_types:
        return
...
def _load_unified_primary(
    ...
    # Tied checkpoints can drop a duplicate embedding before external data is loaded.
    # MiniCPM5 is untied, so both independent matrices remain by default.
    model = onnx.load(str(primary_path), load_external_data=False)
    info = (
...
def build_quantized_merged_bundle(resolved: dict[str, ResolvedPlan]) -> None:
    ...
    weights_equal = _source_embed_lmhead_equal(source_folder / primary_file)
    share_embed_lmhead = UNIFY_EMBED_LMHEAD and weights_equal
    _configure_embedding_quantization(primary_plan, share_embed_lmhead)
    print(
        "  Embed/lm_head check: "
        + ("identical; sharing enabled." if share_embed_lmhead else
           "not shared; AFFINE_REFINE_V2 will quantize the embedding Gather." if primary_plan.algo == "AFFINE_REFINE_V2" else
           "not shared.")
    )
    ...
    if UNIFY_EMBED_LMHEAD and not weights_equal:
        print("  Embed/lm_head sharing disabled because the exported weight values differ.")
