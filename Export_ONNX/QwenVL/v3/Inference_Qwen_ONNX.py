"""Run standalone Qwen3-VL merged-ONNX inference."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run a merged-ONNX Qwen3-VL inference demo.")
    parser.add_argument(
        "--model-folder",
        type=Path,
    ...
    parser.add_argument(
        "--tokenizer-folder",
        type=Path,
        default=Path.home() / "Downloads" / "Qwen3-VL-2B-Instruct",
...
TEST_IMAGE = [str(_SCRIPT_DIR.parent / "psyduck.png")]
TEST_VIDEO = str(_SCRIPT_DIR.parent / "test_video_8s.mp4")
...
def build_assistant_prompt_prefix() -> str:
    ...
    if ENABLE_THINKING:
        raise RuntimeError("Qwen3-VL Instruct does not support enable_thinking in its chat template.")
...
print("\nStart running the Qwen3-VL LLM by merged ONNXRuntime.\nNow loading . . . it could cost minutes.")
