...
class PhiAttention(nn.Module):
    ...
    def forward(
        ...
        return self.dense(torch.matmul(nn.functional.softmax(torch.matmul(
            torch.cat((query_states * rotary_pos_emb_cos + rotate_half(query_states) * rotary_pos_emb_sin, query_pass), dim=-1), key_states.transpose(1, 2)) * self.head_dim_factor + attention_mask, dim=-1, dtype=torch.float32),
                                       value_states).transpose(0, 1).reshape(ids_len, self.hidden_size).contiguous()), key_states.half(), value_states.half()
...
class PhiForCausalLM(PhiPreTrainedModel):
    ...
    def forward(
        ...
        for i in range(self.num_layers):
            hidden_states, self.save_kv_0[i], self.save_kv_1[i] = self.model.layers[i](
                hidden_states=hidden_states,
                attention_mask=attention_mask,
