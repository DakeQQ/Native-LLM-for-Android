...
class Phi3MLP(nn.Module):
    ...
    def forward(self, hidden_states: torch.FloatTensor) -> torch.FloatTensor:
        gate, up_states = self.gate_up_proj(hidden_states).chunk(2, dim=-1)
...
class Phi3Attention(nn.Module):
    ...
    def forward(
        ...
        value_states = torch.cat((past_value_states, qkv[..., self.query_pos + self.kv_factor :].half().view(ids_len, self.num_key_value_heads, self.head_dim).transpose(0, 1)), dim=-2)
        return self.o_proj(torch.matmul(nn.functional.softmax(torch.matmul(query_states * rotary_pos_emb_cos + rotate_half(query_states) * rotary_pos_emb_sin,
                                                                           key_states.transpose(1, 2).float()) * self.head_dim_factor + attention_mask, dim=-1, dtype=torch.float32), value_states.float()).transpose(0, 1).reshape(ids_len, self.hidden_size).contiguous()), key_states, value_states
...
class Phi3ForCausalLM(Phi3PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.expand_space_A = torch.zeros((self.max_seq_len, self.hidden_size), dtype=torch.float16)
        self.expand_space_B = torch.zeros((self.num_layers, self.num_key_value_heads, self.max_seq_len, self.head_dim), dtype=torch.float16)
        # Initialize weights and apply final processing
        self.post_init()
    ...
    def forward(
        ...
        for i in range(self.num_layers):
            hidden_states, self.save_key[i], self.save_value[i] = self.model.layers[i](
                ...
                ids_len=ids_len)
        expand_space_B = self.expand_space_B[:, :, kv_seq_len:, :]
        return (torch.cat((hidden_states.half(), self.expand_space_A[ids_len:, :]), dim=-2),
                torch.cat((torch.stack(self.save_key), expand_space_B), dim=-2),
                torch.cat((torch.stack(self.save_value), expand_space_B), dim=-2))
