...
class PhiAttention(nn.Module):
    ...
    def forward(
        ...
        return self.dense(torch.matmul(nn.functional.softmax(torch.matmul(
            torch.cat((query_states * rotary_pos_emb_cos + rotate_half(query_states) * rotary_pos_emb_sin, query_pass), dim=-1), key_states.transpose(1, 2).float()) * self.head_dim_factor + attention_mask, dim=-1, dtype=torch.float32).to(torch.float32),
                                       value_states.float()).transpose(0, 1).reshape(ids_len, self.hidden_size).contiguous()), key_states, value_states
