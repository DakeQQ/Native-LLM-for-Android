...
# Load the model
shutil.copyfile('./modeling_modified_A/modeling_phi3.py', model_folder_path + "/modeling_phi3.py")
...
max_seq_len = 1024  # Please modify the same variable, which declared in the modified modeling_phi.py on line 723, at the same time.
...
# Reload for part B
shutil.copyfile('./modeling_modified_B/modeling_phi3.py', model_folder_path + "/modeling_phi3.py")
model = AutoModelForCausalLM.from_pretrained(model_folder_path, torch_dtype=torch.float32, device_map='cpu', trust_remote_code=True, low_cpu_mem_usage=True).eval()
model.register_buffer('cos_rotary_pos_emb', cos_rotary_pos_emb)
model.register_buffer('sin_rotary_pos_emb', sin_rotary_pos_emb)
