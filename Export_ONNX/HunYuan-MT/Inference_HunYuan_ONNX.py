...
def parse_args():
    parser = argparse.ArgumentParser(description="Run the HunYuan-MT merged-ONNX inference demo.")
    parser.add_argument(
        "--model-folder",
        type=Path,
        default=Path(__file__).resolve().parent / "Hunyuan_Optimized",
        help="Folder containing merged ONNX graphs and LLM_SharedInitializers.onnx(.data).",
    )
    parser.add_argument(
        "--tokenizer-folder",
        type=Path,
        default=Path.home() / "Downloads" / "Hy-MT2-1.8B",
    ...
    parser.add_argument(
        "--source-language",
        default="English",
        help="Source language name, Chinese name, or abbreviation.",
    )
    parser.add_argument(
        "--target-language",
        default="Chinese",
        help="Target language name, Chinese name, or abbreviation.",
    )
    parser.add_argument(
        "--max-new-tokens",
        type=int,
...
def _language_names(language):
    aliases = {}
    for abbreviation, english_name, chinese_name in _LANGUAGE_ROWS:
        names = (english_name, chinese_name)
        aliases[abbreviation.lower()] = names
        aliases[english_name.lower()] = names
        aliases[chinese_name] = names
    aliases.update({
        "tagalog": aliases["tl"],
        "farsi": aliases["fa"],
        "myanmar": aliases["my"],
        "uighur": aliases["ug"],
        "traditional chinese": aliases["zh-hant"],
        "cambodian": aliases["km"],
    })
    return aliases.get(str(language).strip().lower())


def build_translation_instruction(text, source_language, target_language):
    source_names = _language_names(source_language)
    target_names = _language_names(target_language)
    if not text:
    ...
    labels = (str(source_language), str(target_language))
    has_cjk_label = any(any("\u4e00" <= char <= "\u9fff" for char in label) for label in labels)
    involves_chinese = source_names[0].startswith("Chinese") or target_names[0].startswith("Chinese")
    if has_cjk_label or involves_chinese:
        return (
            f"将以下文本翻译为{target_names[1]}，注意只需要输出翻译后的结果，不要额外解释：\n\n"
            f"{text}"
        )
    return (
        f"Translate the following text into {target_names[0]}. "
...
def load_metadata_carrier(model_path):
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Re-export with HunYuan_MT_Export.py or copy the metadata carrier into the model folder."
        ) from exc

    metadata = meta_session.get_modelmeta().custom_metadata_map
    if not metadata.get('native_llm_metadata_version'):
        raise RuntimeError(
            "LLM_Metadata.onnx carries no Native-LLM metadata_props. Re-export with HunYuan_MT_Export.py."
        )
    return metadata
...
def load_model_file_names(meta):
    ...
    if missing:
        raise RuntimeError(
            ...
            "Re-export with HunYuan_MT_Export.py or re-quantize with Optimize_ONNX.py. Missing: "
            + ", ".join(missing)
        )
...
print(
    '\nStart running HunYuan-MT by ONNX Runtime.\n'
...
def _merged_stop_tokens(meta):
    ...
    if not stops:
        stops = [int(v) for k in ('chat_eos_id', 'chat_eot_id') for v in (meta.get(k),) if v]
    return set(stops)
...
def run_merged_iobinding(folder, meta, strategy, model_file_names):
    ...
    tokens = tokenizer(prompt, add_special_tokens=False, return_tensors='np')['input_ids']
...
if _missing_shared_files:
    raise RuntimeError(
        ...
        f"{onnx_folder}. This runtime is merged-only; re-export with HunYuan_MT_Export.py or "
