"""Export HunYuan-MT split and merged ONNX graphs for on-device inference."""
...
# Settings
MODEL_PATH                     = str(Path.home() / "Downloads" / "Hy-MT2-1.8B")  # HF checkpoint folder.
...
EXTRA_STOP_TOKEN_IDS           = (127960, 127967)                    # Additional HunYuan-MT stop IDs; out-of-vocab IDs are ignored.
...
RAW_ONNX_DIR                   = SCRIPT_DIR / "Hunyuan_ONNX_Raw"
ONNX_DIR                       = SCRIPT_DIR / "Hunyuan_ONNX"
ONNX_DIR.mkdir(parents=True, exist_ok=True)
INFERENCE_SCRIPT               = SCRIPT_DIR / "Inference_HunYuan_ONNX.py"
MODEL_FILE_NAMES = {
...
class LLM_MAIN(torch.nn.Module):
    ...
    def forward(self, *all_inputs):
        ...
        for i, layer in enumerate(self.llm.model.layers):
            ...
            # HunYuan applies RoPE before its per-head Q/K RMSNorm.
            qk_rot = qk * rotary_pos_emb_cos + self._rotate_half(qk) * rotary_pos_emb_sin
            qk_rot = self._rms_norm(qk_rot, self.qk_norm_scale, self.qk_rms_norm_eps) * layer.self_attn.qk_norm_weight
...
def collect_special_token_ids(model_path):
    ...
    try:
        ...
        candidates = {
            "chat_bos_id":            special_id("<｜hy_begin▁of▁sentence｜>"),
            "chat_eos_id":            special_id("<｜hy_end▁of▁sentence｜>"),
            "chat_user_id":           special_id("<｜hy_User｜>"),
            "chat_assistant_id":      special_id("<｜hy_Assistant｜>"),
            "chat_eot_id":            special_id("<｜hy_EOT｜>"),
            "chat_assistant_end_id":  special_id("<｜hy_place▁holder▁no▁2｜>"),
            "chat_system_end_id":     special_id("<｜hy_place▁holder▁no▁3｜>"),
            "chat_startoftext_id":    special_id("<|startoftext|>"),
            "chat_extra_0_id":        special_id("<|extra_0|>"),
...
if __name__ == "__main__" and DO_EXPORT:
    ...
    with (torch.inference_mode()):
        ...
        if isinstance(_eos, (list, tuple)):
            _stop_ids = {int(e) for e in _eos}
        elif _eos is not None:
            _stop_ids = {int(_eos)}
        else:
            _stop_ids = set()
        _stop_ids.update(token_id for token_id in EXTRA_STOP_TOKEN_IDS if token_id < vocab_size)
        _eos_ids = ",".join(str(token_id) for token_id in sorted(_stop_ids)) or None
        model_config_meta = {
...
def run_inference_demo():
    ...
    print(
        f"\n{status}\n\n"
        "Start running HunYuan-MT by ONNX Runtime via Inference_HunYuan_ONNX.py.\n"
