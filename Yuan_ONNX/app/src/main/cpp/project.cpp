                                                     ...
                                                     jboolean clear)
{
    if (add_prompt)
    {
        ...
        get_ids.insert(get_ids.begin(), {77187}); // Chat prompt head
        get_ids.insert(get_ids.end(), {77187});   // Chat prompt tail
    ...
    }
    {
        ...
        std::vector<half> before_hidden_states_temp;
        ...
        if (add_prompt)
        {
            ...
            ort_runtime_A->CreateTensorWithDataAsOrtValue(
                ...
                &input_tensors_A[3]);
            before_hidden_states_temp.resize(before_hidden_states_size, half(0.f));
            ort_runtime_A->CreateTensorWithDataAsOrtValue(
                memory_info,
                reinterpret_cast<void *>(before_hidden_states_temp.data()), before_hidden_states_buffer_size,
        ...
        else
        {
            ort_runtime_A->CreateTensorWithDataAsOrtValue(
                memory_info,
                reinterpret_cast<void *>(reinterpret_cast<half *>(key_states)), past_key_values_buffer_size,
                input_dims_A[2].data(), input_dims_A[2].size(), input_types_A[2],
                &input_tensors_A[2]);
            ort_runtime_A->CreateTensorWithDataAsOrtValue(
                memory_info,
                reinterpret_cast<void *>(reinterpret_cast<half *>(value_states)), past_key_values_buffer_size,
                input_dims_A[3].data(), input_dims_A[3].size(), input_types_A[3],
                &input_tensors_A[3]);
            ort_runtime_A->CreateTensorWithDataAsOrtValue(
                memory_info,
                reinterpret_cast<void *>(reinterpret_cast<half *>(before_hidden_states)), before_hidden_states_buffer_size,
                input_dims_A[6].data(), input_dims_A[6].size(), input_types_A[6],
                &input_tensors_A[6]);
        }
        ort_runtime_A->CreateTensorWithDataAsOrtValue(
        ...
        ort_runtime_A->CreateTensorWithDataAsOrtValue(
            memory_info,
            reinterpret_cast<void *>(&control_factor), sizeof(int64_t),
    ...
    {
        void *last_hidden_states;
        ort_runtime_A->GetTensorMutableData(output_tensors_A[0], &last_hidden_states);
        ...
        ort_runtime_A->GetTensorMutableData(output_tensors_A[3], &before_hidden_states);
        OrtMemoryInfo *memory_info;
        ...
        ort_runtime_B->CreateTensorWithDataAsOrtValue(
            memory_info,
            reinterpret_cast<void *>(reinterpret_cast<float *>(last_hidden_states)), last_hidden_states_buffer_size,
                                                            ...
                                                            jboolean use_dsp_npu)
{
    ...
    OrtSessionOptions *session_options_A;
    {
        ...
        ort_runtime_A->AddSessionConfigEntry(session_options_A, "session.force_spinning_stop",
                                             "0"); // 1 for low power
        ort_runtime_A->SetSessionGraphOptimizationLevel(session_options_A, ORT_ENABLE_BASIC);  // Use ORT_ENABLE_BASIC to keep calculations in FP16; otherwise, ONNXRuntime will automatically add Cast operators to convert to FP32.
